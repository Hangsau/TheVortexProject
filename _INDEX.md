...
- `plans/B組_池畔可見方向與解剖學動作兩欄核對.md` — **核對完成、三份派工規格已到齊（2026-09-03）** · (B) 組 13 個起跳轉身相位的逐相位「池畔可見方向 ↔ 解剖學動作」兩欄核對稿，是 W10／W11／W12 三份規格的**唯一裁決來源**（執行者不得自行裁決）。三個結構性產出：① 13 相位縮為 **10 個可派工**——`flip`／`wall-rotation` 裁決為剛體空間重定向、**不可用 `joint-local` 表達**，`breaststroke-pullout` 裁決為庫內缺關節層素材（自成 (D) 類），這三個相位**永遠不得**在後續 W 輪順手補上；② 核對過程抓到庫內兩處命名錯誤（`tech.42` 的 `sub-luxation`／`腰椎反弓`，已於 `dd45e88` 修正）；③ §4b 明列剩餘工項與「W10 → W11 → W12 **串行不平行**」的發包順序——W11／W12 的硬性前置條件會 grep 前一批的落地結果，平行發包必然誤判成「前置未落地」
- `plans/W10_起跳段覆蓋派工規格.md` — **已撰寫完成，待發包（`84bbd19`）** · `starts-turns` 四相位（`set-position`／`takeoff`／`flight`／`entry`）＋**零新增 action／muscle**。核心是 `takeoff` 那筆的**後腿方向反轉**：池畔看見的「後腿往後蹬」在解剖學上是髖**伸展**，而蹲踞式起跳的預備姿勢是髖屈曲——同一條腿在同一相位內先後出現相反方向，池畔語言把兩者壓成「蹬」一個字。另兩個做法：`set-position` 寫成「**並存的多種架式，不是單一姿勢**」（不裁定哪種為正），`flight` 寫成「**姿勢達成但不改變彈道**」（飛行中重心軌跡已鎖定，任何動作都只是姿勢調整）
- `plans/W11_牆段覆蓋派工規格.md` — **已撰寫完成，待發包（`8410b3c`）** · `starts-turns` 牆段三相位（`turn-approach`／`wall-contact`／`wall-push-off`）＋零新增 action／muscle。本檔的可複製設計是**擁有權分工表**：`tech.47`／`tech.48` 同時被 W10 D4 與 W11 D3 引用，規格明列「命名更正＋腰椎擇一」歸 W10 入水、「肩複合體合計角度＋流線時序」歸 W11 推蹬，**重述他人擁有的內容即退件**。內容三個更正：頸部「不伸展」是維持不是動作、膝屈曲**深度不是關節動作**（深淺是姿勢參數，髖屈曲在此為「不主動做」）、「手臂舉到接近極限」是肩肱＋肩胛**合計**角度而非單一關節
- `plans/W12_出水段與蝶式入水停頓覆蓋派工規格.md` — **已撰寫完成，待發包（`d0397f3`）** · 跨三個 stroke 的三相位（`starts-turns/breakout`／`udk/breakout`／`fly/entry-pause`）＋零新增 action／muscle，(B) 組最後一份。四個要點：① `starts-turns/breakout` 列九筆 action 但**只有兩筆是「做」**，其餘七筆是「維持」或「不做」——這個結構必須正面寫出，否則讀者會把清單讀成九個要執行的動作；② `udk/breakout` 釘死俯臥下踢＝**髖屈曲**（BK-30／ST-13 的具體化），且 `hip.extension` 的刻意省略必須解釋；③ `fly/entry-pause` 保留 `fly.tech.1`「連續波」與 `fly.tech.21`「入水停頓」的**命名衝突而不裁定**，並禁止由「拇指 45° 切入」反推肩內旋／前臂旋前；④ **`udk/breakout` 的 `derived_from_ids` 前綴是 `starts-turns.` 不是 `udk.`**——全庫沒有 `udk.tech.44`，「順手修正」會把有效引用換成不存在的 ID 並打破 W014
- `plans/movement_coverage_denominator.yaml` — **不是覆蓋率真相源**（該檔第 10 行自己寫明相位集合分歧時以 `_taxonomy.yaml` 的登錄表為準）。它保留的是各相位的證據引文。覆蓋率一律看 `KNOWLEDGE_MAP.md` 的「相位覆蓋」表，由登錄表 59 相位算出：**34/59**（自由式 6/12、仰式 **7/7**、蛙式 8/9、蝶式 9/11、udk 3/7、起跳轉身 1/13）
...
*最後更新：2026-09-03（(B) 組規格輪：兩欄核對＋W10／W11／W12 三份派工規格到齊，涵蓋 10 個起跳轉身相位、零新增 action／muscle，等串行發包；覆蓋維持 34/59）*
