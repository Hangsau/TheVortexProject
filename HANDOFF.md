...
### ▶️ 蛙式 Pullout 補上。覆蓋 54/59 → **55/59**，需求 61 → **62**。**它從來不缺量測，缺的是查找（錯誤 11）**
...
**驗證**：0 ERROR、WARN 604 → **602**、W019 維持 0、**211 passed／8 subtests**、`KNOWLEDGE_MAP.md` 已重生成。
...
**仍然缺的 4 個相位**（`breaststroke-pullout` 已於本日稍後補上，見最上方錯誤 11）：

| 相位 | 類別 | 下一步 |
|---|---|---|
| `udk.kick-initiation` | 缺關節層量測 | 一次水下側視高速拍攝即可（見「下一步建議」的拍攝協議） |
| `udk.down-to-up-transition` | 缺關節層量測 | 同上，同一次拍攝一併回答 |
| `starts-turns.underwater-dolphin-kick` | 缺關節層量測（**重新分類，見錯誤 10**） | 同上。分母維持 59，不除名 |
...
**下一步**：剩餘 7 個未覆蓋相位沒有一個是「再派一批就好」，全部需要決策而不是撰寫——
...
### 最高優先——剩三個相位缺口需要新量測，一次拍攝可全數回答

**先前列的四個「待決事項」現在全部處理完，剩下的只有拍攝**（2026-09-03 兩次更新後重寫，舊清單保留在下方僅供追溯）：
...
| c 之「補量測」半 | ⏳ **從四格降為三格**。`breaststroke-pullout` 是誤分類，翻書就有（見錯誤 11） |
| c 之 `underwater-dolphin-kick` 分母 | ✅ **已決：分母維持 59，除名提案撤回**（見錯誤 10） |

**唯一還在的撰寫類障礙：三個相位缺關節層量測**（`udk.kick-initiation`／`udk.down-to-up-transition`／`starts-turns.underwater-dolphin-kick`）。這三格的既有素材講的是空間軌跡、時序、策略與速度，沒有任何一句可對到關節詞彙。**這是研究工作不是填充工作，不可發包。**

**但它不需要實驗室。** 先前寫「相位化三維關節運動學」是過度規格（同型錯誤已犯四次）。三格都在海豚踢週期上，而海豚踢**基本是平面動作**——所需的是**一次水下側視高速拍攝**：

> 泳者髖、膝、踝三處貼標記，水下側視 ≥100 fps，同一次下水拍三種條件（**出發後、轉身後、途中穩定段**）。讀的是**方向與反轉順序**，不是度數：起踢時哪一個關節先動、下踢轉上踢時三個關節的反轉先後、三種條件下這個順序有沒有變。這三個答案分別落在 `kick-initiation`、`down-to-up-transition`、`underwater-dolphin-kick` 三格。

只有牽涉軸向旋轉的相位才需要三維；這三格不需要。**拍攝本身在一般泳池即可完成，執行者是使用者自己。**

**判別法（本專案已用兩次，第二次用錯一半）**：問「這個相位裡有哪些關節在動」——素材答得出關節名就是問法或**查找**的問題，只答得出距離、速度、時機、策略才是量測問題。**錯誤 11 顯示這條要配一個前置動作才有用：判定「答不出關節名」之前，先實質檢索 `resources/books/`，不能只憑既有 canonical 素材下判斷。**

**可推廣的一條**：**當一項待辦被標成「需要外部資料才能定案」時，先問這筆內容是否真的需要那個精度、以及那個資料是否其實已經在手上。** a 卡在「要幾度」、b 卡在「哪個方向對」、flip 卡在「要不要換框架」、pullout 卡在「以為要新量測」——四者的共同結構是**把可寫的內容誤設成待取得的資料**。四個都在改問法或翻書之後當場可寫。
