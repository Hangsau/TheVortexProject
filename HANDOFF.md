...
結論是**不分**，而且過程推翻了兩件常識。Capelli 等（1998，n＝20 菁英，1.5 公尺／秒）每公尺能量成本實測：自由式 1.23、仰式 1.47、蝶式 1.55、蛙式 1.87 千焦耳／公尺；Barbosa 等（2006，遞增式 200 公尺至力竭，逐次呼吸）排序一致。① 常說的「蝶式最耗能」是錯的，**蛙式才是最貴的**，蝶式在它之前。② 但這不構成按泳式給不同休息的理由——上面測的是**同一絕對速度**下的成本，而組課從來不是叫人四式游一樣快，每一式各自用自己的相對強度（自己的臨界游速、自己那個距離的最佳成績）。**相對強度一旦對齊，能量成本差異已經被配速吸收掉了**，不該在休息上再收一次。泳式差異進入的是「目標時間」那一格，不是「休息」那一格。留一個 🔴：尚無研究比較四式在**各自相對強度下**的恢復動力學，有了要重判這條。
`_sources.yaml` +2（858 → 860），兩筆摘要都經 Europe PMC REST API 取全文逐字核對（PubMed 擋 cookie、Thieme 403，走 `https://www.ebi.ac.uk/europepmc/webservices/rest/search?query=EXT_ID:<PMID>&resultType=core&format=json`）。**Capelli 已出版摘要有一處小數點誤植**：1 m/s 那組把蛙式印成 0.124 kJ·m⁻¹，與同句其他三式差一個數量級、也與 1.5 m/s 的 1.87 不相容，正確應為 1.24；本庫一律只引 1.5 m/s 那組。Barbosa 的 n＝26 是**每式各自的小樣本**（蛙 5／仰 5／蝶 4／自由式 12）且全為國際級，只支持**排序**，不支持逐式點值或比值外推，更不支持兒童與初學者——兩條使用邊界都寫進 `notes`。
驗收：0 ERROR／136 WARN，索引重生 1016 records／860 sources，高確定性無來源 0。`KNOWLEDGE_MAP.md` 無異動是對的（加欄位不是加節點）。

**同一批第二件：`generator.anchor_tiers` 的 tier_c 🔴 結案，但結案方式不是補係數，是換模型 ＋ 改分級。**
原文寫「只有其他單一距離成績 → 用距離—時間關係外推到 400；⚠️ 外推係數本庫尚未查證」。查完之後三個結論：
① **單點外推在數學上就不成立**，不是係數沒查到。兩點法直線與 log-log 冪次**都是兩參數模型**，一個計時點少一條方程式。要硬做只能借族群平均的衰退指數——但個體的「短距∶長距」比正是訓練要改變的東西，借族群值等於把要量的訊號抹掉。**單點已從 tier_c 移到 tier_d**（自校準）或走 embedded_test。
② **有兩點時係數根本不必登錄**，它是逐人現場擬合的：k＝(log T₂ − log T₁)÷(log D₂ − log D₁)，T₄₀₀＝T₂×(400÷D₂)^k。跑步界那個 1.06 不該搬進來——那是跑步世界紀錄的族群擬合值。
③ **模型要換成 log-log，不能用兩點法直線外推。** Osiecki 等（2014，n＝8，19±1.1 歲受訓男性，25 公尺池，50／100／200／400 全部實測）逐一比對：用 100＋200 預測 400，**直線法偏差 −14.25 秒**（p＝0.003，Bland-Altman 不一致），**log-log −2.5 秒**（p＝0.382，一致）；真實 400 平均 276.3 秒，即直線法快了約 5%、log-log 約 0.9%。用 50＋100 預測 200 同方向（−5.52／p＝0.03 vs −0.23／p＝0.456）。**偏快是最危險的方向**——目標時間比實際能力快，整組提早掉速、退出規則一直觸發，看起來像「今天狀況差」，其實是錨點推導錯了。
這跟本檔既有立場是同一件事的兩個切面：`distance_choice_rule_zh` 已據 Toubekis 等（2006）寫過「兩點不可換 50／100，否則臨界速度顯著高於乳酸閾值速度」，那是在**閾值效度**上看到的；這次是在**400 預測誤差**上再看到一次，方向相同——短距離抬高速度—距離直線的斜率。
🔴 **保留**：n＝8、單一研究、非索引期刊（JEPonline，ASEP 官方站取全文 PDF 逐頁核對）、全為自由式青年男性；兒童、初學者、蝶蛙仰皆無資料。**tier_c 因此永遠標「外推」，不因換了模型就升級成「估計」。**
`_sources.yaml` 860 → 861。驗收：0 ERROR／136 WARN（加 `source_ids` 前一度 137——`extrapolation_source_ids` 這種自創欄名驗證器不認，會把新來源判成無人引用的孤兒，欄名必須是 `source`／`source_ids`）。索引 1016 records／861 sources。

**這批的起因值得記一筆**：swim-coach 的 `vendor/vortex` submodule 釘在 `6389b7a`，落後 master `aa708ee` **208 個 commit**，看不到 `set-design.yaml` 與 `decisions.yaml`。於是 swim-coach 那側的「游泳文獻沒有依段落給休息秒數這種表」這類**否定型結論**——寫入當時為真、submodule 更新前已為假——被當成事實，害一個 178 行的研究章節重複推導 Vortex 前一天就做完而且更嚴謹的東西，還憑空標了一個不存在的 🔴 缺口。Vortex 這側的教訓只有一條：**canonical 推進後，下游 submodule 沒跟上等於沒發生**，所以本 repo 的 push 不是收尾裝飾，是讓三端（canonical／swim-coach vendor／my-site data）同源的唯一途徑。
...
網頁已驗 309 站內連結／9 頁、篩選／搜尋／深連結與 320–1440px；my-site `476089d` CI 成功，
公開頁重跑同一驗收全通過。使用者明確授權推送後，W25 `6894568` 與交接 `d9b14b6` 已進 master；
同步 [CI 34701654801](https://github.com/Hangsau/TheVortexProject/actions/runs/34701654801) 成功，公開資料無差異。

## 下一步建議（E組）

E 組 W18–W25 已完成實作、驗收、推送與同步；沒有待推送的 E 組工作。
問題內容若需補充，先看現有缺口與候選裁決，不以湊滿四欄為目的增加介入。
...
**查證結果先行**：筆記有三處要改才收。① 它斷言頭髖連動「跟速度完全無關」——Alves 1999 逐字寫抬頭的不良效果會隨速度提高而在「技術較熟練」那組被削弱、但另一組不會，所以結論（放慢沒用）成立、機制敘述要改寫，不能寫成絕對。② 網球夾下巴 drill，筆記說「下巴縮太低球也會掉，同時避免兩種極端」——Swimming World 原文只寫抬頭會掉球，沒有這個說法，已剔除並在來源 notes 標為「整合時抓到的加料」。③ 蝶式換氣時機筆記寫「推水最後三分之一」並歸給 USMS 論壇，Lepinski 原文是 "Breathe when your arms pull under your torso"，採原文措辭。論壇討論串一律不登錄成來源（那正是 `src.swimswam-a/-b` 被立墓碑的型態），只收具名具日期的文章。

**新增**：`_sources.yaml` +5 筆（Stallman 2010 蛙式運動經濟性會議論文、Alves 1999 蝶式吸氣運動學會議論文、USMS 蛙式身體位置指南、Swimming World 網球 drill、Lepinski 2020 USMS 蝶式專欄），每筆 notes 都寫死使用邊界。`teaching-errors.yaml` +2 條：`breast.err18`「游慢一點就不會抬頭壓髖」、`fly.err17`「蝶式換氣沉臀只要調速度就會好」，兩條的 `correct_concept` 都指向同一件事——用外部限制把抬頭這個選項物理性拿掉，比口頭提醒有效。`breast.err13` 補上 🟢 證據層（原本只有 🟠 且自承「本檔未記錄觀察者、樣本與量測方式」）。Drills +3：`Br35` 網球夾下巴蛙式、`Br36` 水中步行蛙式划手、`Fl32` 憋氣對照換氣蝶式（照 Alves 實驗設計改寫成可操作的 A/B 對照），9 軸指紋齊全。
...
週期決策工具第一版已從流量中斷／重啟後的本機檔案接續完成並發布。上一階段資料、程式、執行紀錄均有保存；中斷點為 2026-09-10 08:05 的 `zones.yaml` 同步比對誤報。兩個 repo 的 29 個原始變更檔已另外備份在 `C:/claudehome/tmp/periodization-recovery-2026-09-10/`，有 ZIP 與逐檔 SHA-256。

新增 `canonical/periodization/decisions.yaml`：八個優先分支、三個情境、1–7 課次分配、共用觀察欄與兩篇新研究摘要。規則是 🔵 專案操作模型，未聲稱已驗證訓練成效。先處理症狀、當日疲勞及反覆退步，再處理近期事件、基線、轉移、進步與維持。修正 RPE／計時方向、不再將所有賽配速對到同一區，並保留疲勞學習研究的任務限制。

週期化目錄由 42 筆補齊既有 GAS／flexibility，再加入四個決策節點，共 48 個穩定 ID；地圖工具改為遞迴收集 ID，索引重生為 927 records／831 sources。my-site 共用這份資料生成互動表、靜態決策表、範例與資料庫入口。`swim-coach` 私人規則不在本批。

驗收：canonical 0 ERROR／140 WARN（原 142）；六份 YAML 經既有中文折行清理後一致。Hugo 170 頁成功；本機 21 項瀏覽器驗收、8,820 條件組合、四個新目錄落點全通過，已目視手機表單與桌面結果。詳見 `plans/periodization-planner-2026-09-10.md`；網站證據在 `my-site/research/periodization-planner-2026-09-10/`。

## 下一步建議（本批週期工具）

本批已完成：canonical `748756ffc4867767ba0399ce6f0a2ba7a655f670` 與 my-site `a37790963e2922b9f2cbd80d2017287ee67a2c90` 已推送，兩端 CI 均按完整 SHA 核對成功；正式站 21/21、8,820 組驗收通過，公開 CSS／JS 與本機一致。入口 https://hangsau.github.io/cortex/vortex/periodization/#planner 。之後依實際使用回饋調整決策，不再從零重建。功能不儲存表單，需用「複製完整安排」保存個案結果；沒有背景研究排程。下方保留其他研究線的歷史待辦。

## 先前狀態（2026-09-06）

### ✅ **Gonjo 族重建：一篇不存在的論文（錯誤 81）、一篇認錯人的論文（錯誤 82）、一個寫反的結論（錯誤 83）——外加驗證器的第二條引用通道一直是盲的**

`src.gonjo`（「Gonjo 2020; Maglischo 2003」）拆解時發現這不是一個複合鍵的問題，而是**整個 Gonjo 族的歸屬是亂的**：註冊表裡有 8 個 Gonjo 開頭的 id，橫跨四篇真論文與一篇不存在的論文，本庫在 17 個引用點上把它們互相串錯。已全族重建。

`_sources.yaml` 827 → **829**（3 筆新登錄 verified、1 筆原地升 verified、2 筆升 verified、7 筆立墓碑）。ERROR 0，WARN **144 → 142**。

**四篇真論文的分工，本來全被壓成「Gonjo 說」。** 拆完才看得出沒有任何一篇能支撐本庫掛在它們名下的多數主張：

| 來源 | 是什麼 | 量了什麼 | **沒有**量什麼 |
|---|---|---|---|
| `src.gonjo-2016`（Sanders RH, **Gonjo T**, McCabe CB, JSSM 15(1):158-166, PMID 26957939） | 數位化**信度**方法學研究，n=3 | 重複數位化的一致性 r=0.93–0.99 | **沒有速度比較**，撐不起任何「隨速度變／不變」的主張 |
| `src.gonjo-2020-upper-body-kinematics`（J Biomech 98:109452, PMID 31708239） | 十名男性**競技**泳者，自由式 vs 仰式 | 肩滾轉 52.88±4.89 vs 49.73±5.73°、髖滾轉 33.79±6.07 vs 39.83±7.25°、SL、SF、放手與恢復期時長 | **只測一個速度條件**；**沒有量手部軌跡形狀**；**沒有做個體差異分析** |
| `src.gonjo-2021-body-roll`（Sci Rep, PMID 33436944） | 同十人，四種強度（最大速度 83–100%） | 幅度隨強度的變化、峰值時點的移動 | — |
| `src.gonjo-2023`（Sports Biomech 22(12):1722-1733, PMID 34380363） | 100 m 蛙式，精英 vs 次精英 | **分段速度**（游進 7–11%、滑行約 13%、pull-out 無差異） | **沒有量划距、沒有量划頻、沒有做疲勞穩定性分析** |

**錯誤 81 — 「Gonjo 2020 個體差異研究」這篇論文不存在。** 本庫在三處（`free.err24`、`back.err2`、`back.tech.11`）寫「現代研究確認菁英自由式選手手部軌跡是相對直線的後推（Gonjo 2020 個體差異研究的核心發現之一）」，全標 🟢。Gonjo T 在 2020 年唯一的游泳運動學論文是 PMID 31708239，它量的是滾轉角與相位時長，**手部軌跡與個體差異兩件事都沒碰**。這是本專案第二次抓到「研究本身是幻覺」（前一次是錯誤 69）。三處的 🟢 主張連同 evidence 區塊已整段刪除，改標 🔵 並在原地寫下更正註記。

**錯誤 82 — `src.gonjo-2016` 認錯論文，而且被拿去撐一個它方法上不可能回答的問題。** 該登錄被用來支持「仰式在不同速度下肩旋轉與髖旋轉幅度不隨速度改變」，並被寫成「2016 的發現由 2021 延續」的一條研究脈絡。實際那篇是 Sanders 主筆的**數位化信度**研究，n=3、單一速度、沒有速度間比較——那條 2016→2021 的傳承是編出來的。已升 verified 並寫死使用邊界；本輪後它不再被任何條目引用（W008 +1），照 `src.gonjo-2018` 前例保留不立墓碑（它是真論文、id 字面值是穩定鍵）。

**錯誤 83 — 結論寫反：時序前移是自由式的特徵，不是仰式的。** `back.tech.2` 寫「仰式在增加強度時……轉移旋轉峰值至划手週期前段」。Sci Rep 2021 原文逐字：swimmers "shifted the timing of WBR and hip roll peak toward the beginning of the cycle when increasing the intensity **in front crawl**"；仰式是**維持**時序。這是錯誤 58、60、62、63、65、66、77 之後**第八次**證明引用的結論方向可能與原文相反。

**錯誤 84 — 拿速度冒充划距。** `src.gonjo-2023` 在四個點被引為「划距（SL）穩定」與「疲勞下技術穩定性」。它量的是分段速度，**划距與划頻一個都沒量**。四處全部改寫成速度語言；唯一原本就對的是「pull-out 差異反而不顯著」。`breast.tech.22` 整條從「精英優勢在划距穩定性」改成「差距在游進段與滑行段速度，pull-out 反而不是」。

**錯誤 85 — 引用自我矛盾：被引的那篇直接反對這句話。** `l-indicators:371` 寫「仰式……划距穩定」並掛 Gonjo 2021。該文 Table 1 顯示仰式划距隨強度**逐級下降**：2.59 → 2.52 → 2.29 → 2.07 m/cycle。已把「划距穩定」從指標刪除。

**錯誤 86 — 引錯泳式。** `free.L6.stability`（自由式指標）掛的是一篇**蛙式**研究。

**錯誤 87 — 「精英」灌水。** PMID 31708239 與 33436944 的受試都寫 "ten male **competitive** swimmers"，本庫多處寫成「精英」。已全數改「競技」，含 `back.tech.1` 標題與 `teaching-errors` 的一條 `cross_ref`。

**錯誤 88 — 推論冒充實測。** `back.err1` 的「高於 40° 是群體常態」是由 mean−1SD（49.73−5.73≈44）反推出來的，原文沒有這句。已刪。

**錯誤 89 — 確定性標記錯置第六例（本輪新變體）。** `back.tech.5` 的 mechanism 是兩條純幾何推導（過度旋轉的兩種代價），卻標 🟢 並掛 `src.gonjo-2020-c`——那篇**阻力與划頻一個都沒量**。前五例都是「🔵 區塊塞實測數字」，這次是反過來：**🟢 標在一段推導上，而附的來源什麼都沒測**。同型檢查應該雙向做。

**錯誤 90 — 二手引用被寫成第一手量測。** `stroke-demands` 的入水期滾轉峰值 `measurement_conditions` 把「峰值落在入水期到拉手期的交界」連同 Gonjo 2021 的方法描述一起寫成該研究的量測結果。取回 PMC 全文後解析參考文獻編號（`<ref id="…CR41">`）確認：那句在原文是**討論段引用他人**，出處是 McCabe 等人 2011（J Sports Sci 29(2):115-123, PMID 21120744）。已新登錄 McCabe 2011，並把該 `measurement_conditions` 拆成兩筆（時序歸 McCabe、強度效應歸 Gonjo）。**這個查法可複用**：`efetch db=pmc` 取 XML → 用參考文獻編號回查書目，就能分辨「這篇說 X」與「這篇引別人說 X」。

**錯誤 91 — 過度絕對。** 「仰式旋轉幅度完全不縮小」漏掉原文的但書：全身滾轉（WBRBT）幅度在**兩種泳式都**隨強度下降，不變的是肩／髖滾轉。已在三處補上但書。

**🔧 驗證器補強：`measurement_conditions[].source_id` 是第二條引用通道，過去 E015 與 W008 都看不到它。** McCabe 2011 登錄後立刻被誤報成孤兒，才暴露這件事——驗證器全程假設「引用只走 `source_ids`」。後果有兩個方向：① 只在量測條件裡被引用的來源被誤報孤兒，誘導人去刪一筆正在被用的登錄；② **掛在這裡的墓碑完全靜默**，E015 整條防線在 movement 檔上是空的。已修，並用「把一個 `source_id` 改指墓碑」實測確認 E015 會抓（1 ERROR），還原後歸零。

**三個寫進登錄的地雷。** ① **`src.gonjo-2021`（PMID 34009106）與 `src.gonjo-2023`（PMID 34380363）在同一份期刊的同一卷同一期**（Sports Biomech 22(12)）——沒有 PMID 的引用字串無法區分兩者，兩筆的 notes 都已標註。② `src.gonjo-2021` 是浮力／水動力扭矩研究（自由式比仰式大 30–40%），與 `src.gonjo-2021-body-roll`（Sci Rep）是**不同的兩篇 2021**，年份加姓氏不足以定位。③ 另有 Sanders 等人 2019/2020（PMID 31537166, Eur J Sport Sci 20(5):624-632）量到十二名熟練泳者手腕軌跡的**試次間**變異 <3 cm——它是 inter-trial 不是 inter-individual，**救不回錯誤 81 刪掉的個體差異主張**，本輪未登錄，記在此避免下次有人拿它去補洞。

**下一步建議**：複合鍵剩下三筆技術族——`src.arellano`（`teaching-errors:2896`、`:2912`，拆成 Arellano 2013 ＋ ScienceDaily 2024）、`src.pink`（`teaching-errors:536`，拆成 Pink 等人 1991 EMG ＋ 沿用既有 `src.riewald-rodeo-2015`；注意 `src.pink-1991-a`／`-b` 兩個候選 id 另在 `technical-analysis` 六處與 `movement/interventions:509` 被引，要一併對齊）、`src.hayashi`（`l-indicators:553`，拆成既有 `src.hayashi-2025` ＋ `src.strzala-2013-a/-b/-c`）。**本輪的教訓要帶進這三筆**：錯誤 81／82 都不是「引用格式不精確」，是**被引的東西根本不是那件事**，所以每筆都要先確認「這篇量了什麼、沒量什麼」再看本庫寫了什麼——先查歸屬、後查方向。另外**應該回頭做一次雙向的標記掃描**（錯誤 89）：不只找「🔵 塞數字」，也要找「🟢 掛在推導上、附的來源什麼都沒測」。**地雷照舊**：W022 剩 9 筆（全在 `l-indicators.yaml`）不可批次清空；`src.gonjo-2018` 與 `src.gonjo-2016` 都是真論文、不可因 W008 立墓碑；`tools/build_injuries.py` 不可執行；`src.race-club-b/-c/-d` 與 `src.mason-1992-a/-b` 不可憑「疑似同一來源」的 note 合併。零星未結：`teaching-errors:1172` 的 `src.swim-teach-com-a3-performance-enjoy-swimming` 三向複合鍵、`l-indicators:487` 的 `src.mason-1992-a` 行，其餘沿用下方段落清單。

---

### ✅ **水下深度這一整族來源重建：一個平台名冒充文獻（錯誤 79）、一組沒人量過的代價被寫成研究結論（錯誤 80）**

拆解 `src.lyttle`（「Lyttle 1998; Novais et al. 2012」），順帶把整個「最優水下深度」證據族一次清乾淨——這一族的三個引用點（`udk.tech.2`、`starts-turns.tech.32`、`udk.err7`）共用同一組數字，只修複合鍵那一個點會留下兩個不一致的版本。

`_sources.yaml` 825 → **827**（2 筆新登錄 verified、4 筆立墓碑）。ERROR 0，WARN 144，皆未變動。

**三篇文獻各自的位置，本來全被壓平了。** 拆完才看得出這三份研究方法完全不同、取樣深度也完全不同，本庫卻把它們的數字混在同一句裡：

| 來源 | 方法 | 取樣深度 | 能支持的 |
|---|---|---|---|
| Lyttle 等人 1998（J Swim Res 13:15-22） | 真人拖曳測力，1.6–3.1 m/s | 水面、0.2、0.4、0.6 m | 0.4 與 0.6 m 降阻 10–20%，0.2 m 降 7–14%；建議約 0.4 m |
| Vennell 等人 2006（J Biomech 39(4):664-671，PMID 16439236） | 假人拖曳 | 至 1.0 m | 波浪阻力在 1.7 m/s 佔總阻力 50–60%；壓到 5% 以下的門檻**隨速度變動**（1 m/s 需 >0.5 m、2 m/s 需 >0.7 m） |
| Novais 等人 2012（J Hum Kinet 33:55-62，PMID 23487502） | 三維 CFD 模擬 | 0–1.0 m，每 0.25 m | 阻力隨深度下降，**超過 0.75 m 後幾乎不再變化** |

**錯誤 79 — 「ScienceDirect 波浪阻力研究」不是文獻。** `src.sciencedirect-wave-drag-study` 的顯示字串只寫了內容聚合平台的名字，那上面有數十萬篇論文，這個字串無法定位任何一篇；它卻掛在一條標 🟢 的具體數值主張上。循主張回溯確認是 Vennell 等人 2006，已新登錄並立墓碑。同時修掉本庫抄它時砍掉的條件：原文的 5% 門檻是**速度相依**的（1 m/s 要 0.5 m、2 m/s 要 0.7 m），本庫寫成「≥ 0.7 m 波浪阻力 < 5%」，等於把一半的條件刪掉。

**錯誤 80 — 「過深反而增加上浮耗時、出水角度變差」沒有任何一篇量過。** 這句話在三個引用點都以 🟢 或 🔵 的姿態出現，並掛在 Novais 2012 名下（`> 0.75 m 無額外效益，反增上浮時間`）。Novais 2012 是純 CFD 阻力模擬，**沒有真人、沒有量上浮時間、沒有量出水角度、沒有量能耗**；Lyttle 與 Vennell 也都只量阻力。這是一句在物理上聽起來合理、教練也普遍相信、但在本庫引的三份來源裡完全找不到依據的話。已改寫成明說「這可以當取捨提示，不能當已證實的代價」。

**另外三項校正。** ① `0.6 m 阻力降約 20%（2.2 m/s 測量）` 的括號限定查不到出處——Lyttle 測的是 1.6/1.9/2.2/2.5/2.8/3.1 六個速度，10–20% 是跨速度的範圍不是 2.2 m/s 的單點值；② 本庫兩處把 Lyttle 的結果寫成「**波浪**阻力降低 20%」，Lyttle 用測力計量的是**總**阻力，沒有拆出波浪阻力分量，波浪佔比要引 Vennell；③ `udk.tech.2` 的 `mechanism` 標 🔵 卻塞滿實測數字（0.4 m／20%／0.75 m），是錯誤 73、74、77 同一型的標記錯置，已把數字全部下放到有來源的 evidence 行。

**合併與墓碑。** `src.novais-2012-a` 與 `-b` 的「疑似同一文獻」註記查證屬實——PubMed 以 `Novais ML[au] AND 2012[dp]` 查詢只回一筆（PMID 23487502），兩者一併立墓碑，統一改指新登錄的 `src.novais-2012-glide-depth-cfd`。`src.lyttle-1998` 由 unverified 升 verified：該刊不在 PubMed，無 PMID 也無 DOI，書目由西澳大學機構典藏、Novais 2012 全文的參考文獻條目、該刊摘要三個管道交叉確認。

**兩個寫進登錄的地雷。** ① 同一組作者另有 1999 年會議論文（Optimal depth for streamlined gliding, BMS VIII, pp.165-170）與 2000 年 J Sports Sci 論文（`src.lyttle-2000`），三篇常被混引，Novais 2012 文中就分別引了 1998 與 1999 兩篇講不同的事。② Novais 2012 有一個本庫從未提及、且與直覺相反的結果：**最低阻力出現在「水面」條件**、最高在 0.25 m。原因是該條件下模型半出水、濕表面積小，作者自己註明這種姿勢在出發轉身後「不可行」——這個數字不可被拿去論證「貼水面比較快」。

---

### ✅ **`src.benjanuvatra-2007-b` 拆解：PMCID 屬於另一篇，兩個樣本的角度被併成一個區間（錯誤 78）**

原判斷是「疑似與 `-a` 同一文獻，待合併」。取回 PMC3990873 後確認**不是合併而是拆解**：該 PMCID 對應的是 Vantorre J, Chollet D, Seifert L (2014)《Biomechanical Analysis of the Swim-Start: A Review》（J Sports Sci Med 13(2):223-231，PMID 24790473），與 Benjanuvatra 無關。原複合鍵把兩篇壓在一起，還把只屬於後者的 PMCID 掛成全鍵的識別碼，兩個引用點的 `url` 也都指向這個錯的 PMC 頁面。

**錯誤 78 — 兩個不同研究的角度被併成一個區間。** `teaching-errors` 寫「精英選手起跳角度約 **21.25–27.45°**」，看起來像一份研究的區間；實際是兩個樣本各自的均值：27.45 ± 5.99°（Benjanuvatra 等人 2007，對照非精英 39.62 ± 13.19°）與 21.25 ± 5.59°（Wilson & Marino 1983）。`technical-analysis` 同一組數字本來就分成兩行分別掛，`teaching-errors` 卻併成一行——同一份資料在兩個檔案裡呈現不一致，且併起來的版本消去了年代差（1983 vs 2007）與樣本差。已拆成兩行對齊。

**同時刪掉一句掛錯人的結論、補上綜述真正說的事。** 原文那行還附帶「入水的重心水平速度與入水距離是起跑成績的主要預測變數」，掛在該綜述名下——摘要中沒有這個說法。Vantorre 綜述實際的框架是：出發是**一連串妥協**（空中距離要長以減阻 vs 入水姿勢要正確而需要旋轉），區辨高手的因子是反應時間、蹬離期足部施力控制與方向、滑行時間、踢腿轉划手的銜接效率——**都不是角度**。已改寫成第三行 evidence，並在條目裡明說「角度越低越好」不是結論、只是精英樣本的傾向。
...
**取回摘要後發現本庫的引用方向也是錯的。** 原文寫「短距離旋轉幅度明顯小於長距離（衝刺 ~37° vs 400m ~50°）」——那組數字只是**髖**旋轉（36.8° 對 49.9°）。同一份研究裡**肩旋轉沒有跟著變小**（97.7° 對 101.6°），因此軀幹扭轉範圍在衝刺時反而**更大**（78.1° 對 61.3°），三個角速度也全面更高。把髖的變化寫成全身的變化，會推出相反的教學結論：原本的 `correct_concept` 據此說「『打開肩膀』對短距離可能是干擾」，實測顯示該被檢查的是髖有沒有跟著肩一起過轉。已改寫 evidence（🟠 → 🟢）與 `correct_concept`。

另把該條 `physical_reason` 裡沒有來源的角度區間（「45–60°／35–40°」）移除——那是標 🔵 的推導段，推導不產出實測角度；具體數字統一交給下方有來源的 evidence 行承擔（與錯誤 73 同一類型的標記錯置）。

---

### ✅ **陸訓族複合鍵拆解：一個憑空的年齡下界（錯誤 71）、一個掛錯來源的成熟度區間（錯誤 72）、一組定位不到出處的介入數字（錯誤 73）**

| 指標 | 段落起點 | 現在 |
|---|---|---|
| ERROR | 0 | 0 |
| 總 WARN | 144 | 144 |
| `_sources.yaml` 筆數 | 813 | **820**（7 筆新登錄 verified + 4 筆立墓碑） |

拆解 `src.aap-pediatrics-2020-145-6-e20201011-nsca-you-2020`（AAP + NSCA 兩機構被壓成一個鍵）與 `src.mccullough-d`（三個不相干來源壓成一個鍵），順帶清掉 `src.mccullough-b`／`-c` 這兩筆同一篇論文的第三、第四次重複登錄。取回 NSCA 2009 立場聲明全文 PDF、Swimming Australia 陸訓教材全文 PDF、AAP 2020 摘要與家長衛教單，逐句核對 `periodization.dryland.youth` 與 `.flexibility`。

**錯誤 71 — 憑空的年齡下界**
本庫寫「多個機構建議 **5–8 歲**是引入阻力訓練的合適區間」。NSCA 2009 全文明寫的是 *no minimum age requirement*，門檻是「能接受指導、遵守指令、承受訓練壓力」，並舉「大致 7 或 8 歲」為參考；PDF 全文檢索「5 or 6」零命中。AAP 家長衛教單同樣寫 7–8 歲。再加 Swimming Australia 教材與 Europe PMC 片語檢索，**五個管道沒有任何一份給出 5 歲這個下界**。已整段刪除並在條目內留更正說明。

**錯誤 72 — 想法對，來源與數字都錯**
「同一實齡的孩子生理成熟度可差 **4–5 歲**」掛在 AAP／NSCA／Swimming Australia 這組複合鍵下。三份原文全部核對過，**沒有一份出現這個數字**。可回溯的原句是 Lindholm 等人 2024（PLoS One，開放取用）的 *somatic maturity can vary by up to 5 years within a birth year*，且該句本身是轉引 Malina 2011 的骨齡研究。已改掛實際寫出這句話的來源、區間改回原文的「最多約 5 歲」、確定性由 🟢 降 🟡（二手轉引），並在註記寫明 Lindholm 的研究對象是 69 名瑞典 12–13 歲男子冰球員，與游泳及陸訓處方無關。

**錯誤 73 — 掛在一篇不可能是出處的論文名下的介入數字**
`teaching-errors` 踝柔韌性條目寫「6 週訓練研究：柔韌性 p=0.0008、25m 踢板 p=0.40」，標 🔵，evidence 掛 `src.mccullough-c`。McCullough 2009 是 20 名女性泳者的**橫斷相關研究、沒有做任何柔軟度介入**，不可能是這組數字的出處。這組數字在文獻中定位不到，已從機制敘述中移除（機制的質性論點保留），evidence 行改寫成待查紀錄。**另注意標記本身也錯**：具體 p 值被標成 🔵（推導），推導不會產出 p 值。

**同時把可查證的部分補得更具體**：① Swimming Australia 教材原文寫負重動作設給「女生 >13 歲、**男生 >14 歲**」，本庫原本只寫「男生更晚」不給數字；該教材另有本庫完全沒收的三項處方——ASCA 分齡最低反覆次數（6–9 歲 15–18 次／9–12 歲 10–12 次／12–15 歲 8–10 次／15–18 歲 6–8 次）、**16 歲以下不做奧林匹克舉重及衍生動作**、需 4 年以上結構化課程且由合格教練直接督導。② `tools_zh` 的「自體重量 → 藥球 → 彈力帶 → 器械 → 自由重量」五段階梯被當成機構處方；NSCA 原文列的是**並列**的器材清單，只規定「簡單動作先於多關節動作」，已降為 🔵 並註明這是本庫自己的操作版本。③ McCullough 2009 的邊界補齊：n=20（10 名 NCAA 第一級校隊 + 10 名休閒泳者），r=0.509 是該研究**唯一達顯著**的變項，所有人體測量值與垂直跳功率均無顯著相關，且橫斷設計不支持「拉大蹠屈就會踢得快」。④ `src.mccullough-d` 裡的「Pink/Tibone 肩關節基線」查回為 Pink & Tibone 2000（Orthop Clin North Am 31(2):247-261），同鍵內的「通用瑜伽解剖／運動科學共識」則自始不是可定位的來源，已移除。

---

### ✅ **週期化族複合鍵拆解：抓到本專案第一組「憑空生成的統計數字」（錯誤 68）與一篇不存在的文獻（錯誤 69）**

| 指標 | 段落起點 | 現在 |
|---|---|---|
| ERROR | 0 | 0 |
| 總 WARN | 144 | 144 |
| `_sources.yaml` 筆數 | 801 | **813**（12 筆新登錄 verified + 5 筆立墓碑） |

拆解 5 個週期化複合鍵（`src.gonzalez-rave`／`src.seiler`／`src.hellard`／`src.mujika`／`src.pmc8607769`），12 篇構成文獻全部取回原文或摘要逐項核對。**這一輪的收穫不是拆鍵，是拆鍵逼出來的三類錯誤：**

**錯誤 68 — 憑空生成的統計數字（本專案首見，也是最嚴重的一類）**
本庫在五個引用點寫「González-Ravé 2021 系統回顧的 **162 筆 TID** 資料中：金字塔型 89 筆、極化型 65 筆、閾值型 8 筆」，並以此推出「游泳金字塔型比極化型更常見」這個載重結論。查證結果：該文摘要明寫「識別 3487 篇 → 篩到 17 篇 → 全文審查後**最終納入 9 篇**」，全文為 IJSPP 付費牆（Europe PMC、OpenAlex 皆確認無開放版本）。而 `162／89／65／8` 這組數字在 PubMed、Europe PMC 全文檢索、引用該文的論文全文、以及全網精確字串搜尋中**都找不到任何出處**。89+65+8 恰好等於 162 這種內部自洽，正是憑空生成最典型的特徵——它不是抄錯，是生出來的。已整組撤除。

**錯誤 69 — 引用了一篇不存在的文獻**
「衝刺項目選手仍約 87–90% 訓練量落在 Z1（Barbosa et al. 2019）」。以作者、年份、主題交叉檢索 PubMed 全部落空。更關鍵的是：目前唯一查得到的衝刺者 TID 實測（Papadimitriou 2026, PMID 41590965，50 自菁英 n=1）各期 Z1 為 80/60/40/50% 與 85/60/30/40%，**與 87–90% 相距甚遠**，所以這不只是來源查無，連數字方向都站不住。已撤除，改掛可核對的替代來源。

**錯誤 70 — 二手轉引標成一手實測**
「4×50／6×50／8×50 m 可誘發 12–18 mmol/L 血乳酸」掛在 PMC8607769 名下標 🟢。取回全文後確認該數字出現在**該文引言**、且明確標注引自 Kabasakalis 2014/2019/2020 與 Nikolaidis 2016。Terzi 等人自己測的是 4×50 m 與 100 m 的比較。已降為 🟡 並標明轉引。

**修掉錯誤之後，結論反而站得更穩。** 「游泳不吻合極化模型」這個判斷本身是對的，只是原本靠一組假數字撐著。取回 Hellard 2019 全文（PMC6470949，127 名法國菁英泳者、20 個賽季、289 個泳者-賽季）後找到真憑據：86–90% 的量在 ≤4 mmol，但拆開來是 40–44% 在 <2 mmol、**44–46% 就落在 2–4 mmol 這個極化派主張要壓低的閾值區**，而作者自己指出其他耐力運動在該區通常只有 2–16%。原文還給了機制解釋——游泳在 <2 mmol 的速度與划頻遠低於實際比賽，久待該區等於練不像比賽的動作。這比原本的計數更有力，因為它說得出「為什麼游泳不一樣」。

**另外修正三處被放大的敘述**：① Seiler & Kjerland 2006 的受試者本庫寫「11 名**菁英**越野滑雪青少年」，原文是 well-trained／nationally competitive junior（全國賽層級），標題裡的 elite 指的是拿來對照的文獻族群；② `macrocycles_per_year: 2–4` 掛 Hellard 2019 名下標 🟢，但該文只看賽季最佳前 **25 週**，給的是窗內型態（兩個約 14–15 週長 macrocycle／兩個 12–13 週／單一平坦），**未給年度數量**，已降為 🔵 並註明是回推；③ González-Ravé 2022 個案的週量本庫壓成單一「24–90 km」，原文是三個 macrocycle 各 25–79／24–87／25–90 km，已分段呈現。

**零誤差的一組**：`src.mujika` 族三篇（1996 模型化減量 +2.90±1.50%／+3.20±1.70% N=18；2002 雪梨 +2.18±1.50%、男 2.57／女 1.78、99 筆）逐字比對全中，是本輪唯一數字完全正確的族。順帶補上原本沒寫的一句：那 99 筆裡有 8 筆是**變慢**的。
...
1. **讀者那端**：它長得像引用，點進去沒有作者、沒有機構、沒有可回溯的對象。這正是專案規則說的「顯示名稱或『文獻／教練觀測』等級不能代替來源」。
2. **驗證器那端（比較嚴重，因為它 fail-open）**：W011 規定「🟠 必須寫 `observation_basis`」，但它有一個逃生口 `if has_source_info(block): continue`（原意是「已經引了 Race Club 的影像觀察 → 依據可追」）。這兩筆登錄剛好滿足那個逃生口——於是「你必須交代觀察基礎」被一個**內容就是「教練觀測」的登錄**擋掉。循環自證：用「我觀察到的」證明「我交代了觀察基礎」。三個 `teaching-errors` 區塊就是這樣一直維持 W011 綠燈。

處理：6 處引用全部改寫成真的 `observation_basis`（誰觀察／族群／外推邊界）。觀察者一律不編造——本檔沒記錄就寫「本檔沒有記錄觀察者」；外推邊界老實分開「畫面看得出來的」與「對它做的解釋」（例：踢腿拍數數得出來，「六拍製造不必要腿部疲勞」是解釋，沒有代謝或肌電量測）。

**用墓碑不用刪除**。原本直接把兩筆刪掉，後來發現專案自己已有 `retracted` 墓碑機制，用途正是「擋住同一筆被重新登錄」——刪掉等於允許它靜默長回來。改成轉墓碑（W008 不受影響，仍是 3）。

順帶修掉一個**墓碑機制自己的 fail-open**：墓碑仍留在 `_sources.yaml` 裡，所以它**仍在 E005 的合法集合中**，引用一筆已判定不可引用的來源會完全靜默。現在 E015 擋。查了一遍現行資料，實際以 `source_ids` 引用墓碑的區塊是 **0**——注意先前用純文字 grep 掃出 1 筆（`breaststrokers-knee.yaml:89` 的 `src.swimmers-knee-epidemiology-sr`），那是**寫在 `flags.pending_verification` 散文裡的說明**，不是引用；而且 `sync_vortex.py` 會整段剝掉 `pending_verification`，線上實測該 slug 出現 0 次。**掃引用要走 parsed `source_ids`，不要 grep 原文。**

另外補掉一個機器鍵外洩：`technical-analysis.yaml` 的 `fly.tech.30` 把 `src.2024-2025` 寫進了 `observation_basis` 散文，而 `observation_basis` 在 `public` 底下是會上線的。

**三條規則都實測過會叫**（延續 W021 那次的教訓，「0 筆」也可能是「規則沒接上」）：E014 塞一筆 `教練觀測 2019–2021` → 1 筆；E015 把某條 `source_ids` 指向現有墓碑 → 1 筆；W011 逃生口把其中一個區塊還原成舊寫法（`source: 教練觀測`、無 `observation_basis`）→ W011 從 0 變 1。全部驗完即還原，`git diff` 確認 `injuries.yaml` 回到 0 行差異。

E014 判準是**整串相等**（扣掉年份與標點後）：「教練觀測 2024–2025」算，`Aaron Peirsol / Ryan Murphy 水下影像分析（教練觀測）` 不算——後者指名了被觀察的對象與媒材，回得去。全庫掃過只有這一筆帶括號的，不誤傷。

`validate.py` 原本**沒有 module 層的 `import re`**（只有函式內局部 import），這次一併補上。
...
- `------` → `——`：91 處，**全部**在散文中間，沒有例外。
- `--` → `–`：64 處，左右**全部**是數字或 `L`（`57--67秒`、`L2--L4`）。

**兩種樣式各自一致是先做完 pattern 盤點才敢批次取代的**：先按 `(前一字, 連字號串, 後一字)` 分組數過，確認只有 `--` 與 `------` 兩種長度、且用途不重疊，才動手。取代式左側用 `(?<=[^\s-])` 排除 `# ----` 這種註解分隔線（本檔剛好沒有，但其他檔有 23 條，之後要對別的檔動手時這條 lookbehind 不能拿掉）。

**全庫只有這一份檔中鏢**（`canonical/` + `Drills/` 全掃）。其他檔的破折號本來就是 `——`。

### ✅ **canonical 的 `>-` 折行在網站上變成 1995 個句中空格。修在 my-site 的 sync 層，canonical 的寫法不用改——但要知道界線在哪**

YAML 規範把摺疊純量（`>-`）的每個折點接成一個空白，那是為空白分詞的語言訂的規則。中文沒有詞間空白，所以**canonical 每一次為了可讀而折行，讀者就會在句子中間看到一個空格**：線上 `psychology-read` 單頁 848 處、`database` 頁 1244 處，全庫 1995 處。

**已在 my-site 的 `sync_vortex.py` 接掉**（新增 `dump_yaml()`，13 個輸出點全走它），**canonical 這邊不用改、也不要改**——折行是為了讓 YAML 好讀好 diff，空白是序列化副產物。這跟上一輪那條 `|-` 規則不衝突，兩者管的是不同東西，別混：

- **內容自己有行結構（列點、需要保留的換行）→ 必須 `|-`**。`>-` 會把換行折掉，結構在 layout 拿到字串之前就不存在了，那是**資料層的破壞**，下游救不回來。
- **內容本來就是一整段、只是在檔案裡折行 → `>-` 可以留**。那只是**呈現層的雜訊**，sync 層接得掉。

**已補上（2026-09-05）**：`indices/*.json` 與 `KNOWLEDGE_MAP.md` 是 canonical 這側自己產的、沒經過 sync，同一套接合規則已抽成 `tools/textfold.py`，兩個 builder 各自在**唯一的寫檔點**套用。

> **錯誤更正**：上一段把這件事寫成「仍帶著折行空白」，語氣像是一片都是——**錯**。實際套上去只改了 **2 處**（都在 `content_index.json` 的 `l-indicators` 標題），`KNOWLEDGE_MAP.md` **0 處**。原因是這兩個產出物存的多半是 id、標題、標籤，不是整段散文，折點本來就少。修法留著是為了結構上關掉這個洞（往後新增欄位不會再漏），不是因為當時真的壞了 1995 處——那個數字是**網站**的，不是這裡的。

**沒有一起接掉的**：`。 L4`、`IVV （週期` 這種「中文標點 + 空白 + 英數」目前留著（產出物裡 3 處）。規則跟 my-site 的 `sync_vortex.py` 保持一致，兩邊都只認「空白兩側都是中文」——中文夾英文本來就該留空白（`泳鏡平衡 drill`），放寬字元集會把作者有意的排版一起吃掉。要動就兩邊一起動。

### ✅ **`fly.tech.23`（四式疲勞崩潰對比）四項主張裡有三項跟它自己要總結的 🟢 條目互相矛盾。已重寫，並補上 `evidence_from` 讓矛盾在下次能被機器擋住**

這是一條 🔵、無來源的跨式對比表，職責是總結四式各自的崩潰序列。但逐條比對它引用的四個來源條目，**四項裡三項寫反了**：

| | `fly.tech.23` 原本寫的 | 來源條目實際寫的 |
|---|---|---|
| 自由式 | EVF 崩潰是首環 | **SL 先掉**，泳者本能提高 SR 補速度感把問題蓋掉；EVF 排第三（`free.tech.24`） |
| 仰式 | 旋轉幅度縮小 | **髖旋轉增加、肩旋轉維持**——代償本身就是首環的可見形式（`back.tech.24`） |
| 蛙式 | 手腳銜接時機失準 | **insweep 腳速下降**（推進力直接損失），提早划手是接著才發生的（`breast.tech.27`） |
| 蝶式 | 第二踢力量下降 | 一致（`fly.tech.22`） |

三項都不是措辭差異，是**把序列中後段的環節說成首環**，而且方向相反（仰式尤其：原文說「縮小」，來源說的是「增加」）。這種錯誤在教學上直接誤導觀察端——盯錯指標會在真正的首環已經失守之後才察覺。

**重寫後四項全部改對，並且明確寫出「EVF 崩潰在自由式排第三、蝶式排第四，是兩式共同的中後段環節，不是任何一式的首環」**，把原本的錯誤反過來當成一條要防的誤讀。`practical_implication` 同步改成四式各盯各的指標，仰式那條特別標「**不是**肩旋轉是否縮小」。

**兩個結構性改動比內容改動更重要**：

- **補 `evidence_from: [free.tech.24, back.tech.24, breast.tech.27, fly.tech.22]`**。原本這條是 🔵 無來源，證據靠四個子條目承擔卻沒有任何機器可讀的宣告，所以「總結跟被總結的內容矛盾」沒有任何一層會發現。宣告之後 E011 會檢查可解析性，下次改動子條目時這條的依賴關係是看得見的。
- **補 `also_strokes: [free, back, breast]`**。一條四式對比原本只掛在 `stroke: fly` 上，等於**只有蝶式頁的讀者看得到**，其他三式的讀者永遠遇不到它。`also_strokes` 之前只用在 `category: joint`，但 `vortex-stroke.html` 的 `$techs` 累積邏輯（line 18）只看 `.stroke`／`.also_strokes`、不看 category，「跨式通則」chip 也是自動掛的，所以不需要改 layout 就會出現在四式頁。

**同時把 `vortex-stroke.html` 技術卡的 `.summary` 從 `<p>` 直印改成 `richblock.html`**：多數 summary 是單句、輸出跟舊版一模一樣，但這種對比型條目的摘要本身就是四條並列，需要列點結構。

### ✅ **20 條誤區的「物理原因」在網站上是空的——不是渲染 bug，是 `beece5a` 那次補 `observation_basis` 時把 `text:` 一起刪掉了。已逐筆從 `beece5a^` 還原**

線上頁面印出 `<span class=vx-label>物理原因 🟠</span><p></p>`：一個有標題、底下什麼都沒有的區塊。查 canonical，**20 條 `teaching-errors.yaml` 條目的 `physical_reason` 只剩 `certainty` 與 `observation_basis`，`text` 整個不見了**（`free.err3`、`free.err9`、`back.err3/6/9/11/12/15/17`、`breast.err3/9/13/14/16/17`、`fly.err15/16`、`udk.err10`、`starts-turns.err10/13`）。

**根因是 commit `beece5a`（「W011 63 → 42：teaching-errors 21 筆補 observation_basis」）的一個機械性編輯錯誤，20 條全是同一個模式**：那次編輯要把 `text: >-` 換成 `observation_basis: >-`，結果**只刪掉了 `text:` 鍵和它的第一行內容**，第二行以後的續行留在原地，被新插入的 `observation_basis` 收編成尾巴。所以損壞同時是兩面的——`text` 消失，而 `observation_basis` 末尾多出一截不屬於它的殘句。以 `free.err3` 為例，`observation_basis` 結尾掛著「離身體應更寬。用這個 drill 的感覺直接對應到正式游泳，反而是錯誤的遷移。」，讀起來像外推邊界寫到一半跳題。

**還原方式是機械比對不是重寫**：從 `beece5a^` 取出舊檔，逐條抓回 `physical_reason.text` 的原始行，並驗證「舊 text 的第 2 行以後」逐字等於「現行 `observation_basis` 的末 k 行」——**20 筆全部精確吻合**，確認損壞模式單一、內容可完整回復，沒有任何一筆需要我補寫。接著把殘尾從 `observation_basis` 移除、把 `text` 插回 `certainty` 之後。修完全庫 102 個 `physical_reason` 區塊 **0 個空 text**。

**同時在呈現層加了防線**（`vortex-stroke.html` 與 `vortex-database.html`）：標題改成綁在 `.text` 上，不綁在區塊存在上。原本 `{{ with .physical_reason }}` 只要區塊在就印標題，`observation_basis` 是給維護者看的證據基礎、不對外輸出，於是「有區塊但無內容」直接變成讀者眼中的內容漏失。

**這件事的教訓不在這 20 筆，在偵測方式**：這是 2026-05 的損壞，撐到 2026-09 才被發現，而且是靠肉眼看線上頁面看到的，不是靠檢查抓到的。`validate.py` 現有規則檢查的是「有沒有來源」「標記合不合法」，**沒有任何一條檢查「宣告了 certainty 的區塊有沒有實際內容」**。同一類的空殼在別的鍵底下同樣不會被擋。值得加一條「區塊有 `certainty` 就必須有非空的內容鍵」的檢查——注意內容鍵逐檔不同（`text` / `prevalence` / `summary` / `one_line` / `data` 都出現過），寫成硬編 `text` 會製造一整排誤判。

### ⚠️ **本機 `hugo` 建置不能用來驗證列點與分段結構——`core.autocrlf=true` 會讓它跟線上結果不一樣**

追一個「每個列點各自包一個 `<ul>`」的疑似 layout bug 時，發現 template 是對的，錯的是本機環境。`data/vortex/*.yaml` 的**工作目錄副本是 CRLF**（4032 個 CRLF、0 個純 LF），**committed blob 是 LF**（0 CRLF、3996 LF）——`core.autocrlf=true` 造成的。而 **Hugo 的 YAML 解析器把引號純量裡的 `\r` 和 `\n` 當成兩個獨立換行**，k 個折行會變成 2k−1 個換行而不是 k−1 個，於是每個列點之間都多出一個空行、被 `richblock.html` 正確地判成「段落結束」而各自封一個 `<ul>`。CI 在 Linux 上讀 LF，輸出是正確的單一 `<ul>`（抓線上蛙式頁確認過，HTTP 200）。

**通則：凡是與換行、分段、列點有關的呈現驗收，一律以線上頁面為準，本機建置只能用來看有沒有 build error。**
...
**這條軸的正確定性**：不是「補 51 個欄位」，是**「4 筆各自等一份特定的原始研究」**，其中至少 1 筆等的研究學界已明文說沒人做過。**上一輪試點的結論在本輪重現且更強**——查文獻的產出是校正與封閉，不是升級。**建議下一步不要再排 `action_status`**（4 筆已逐筆查完，短期不會有新文獻），改做仍有實料的 psychology 70 筆內容軸——**但開工形態見下方錯誤 34，不是接線**。

### ⚠️ **錯誤 34：我上一段（同一輪、前一個 commit）把下一步寫成「psychology 70 筆的內容軸」，方向對，但沒寫出它第一步卡在哪——照字面開工的人會先去把 `l_levels` 轉成邊，那會同時踩到錯誤 18 和「不要為了數字造關聯」**

寫完上一段後實際去把 `psychology.yaml` 攤開盤點，結論要補上：**心理層 70 筆（8 個 theme ＋ 62 個 concept）整檔沒有 `links:`，也沒有 `cross_ref_ids:`**——不是「有欄位但空著」，是連承載邊的鍵都不存在。這就是 70 筆全數掛在孤兒名單上的原因，也代表**這一章不存在「漏接」這種東西可撿**。

檔裡確實有三種看起來像關聯的東西，逐一查過，**三種都沒有合法目標**：

- **`bridge_zh`（7 個 theme 有）不是引用，是章與章之間的敘事轉場。** 實際內容長這樣：`psych.fear` 的是「恐懼這道門過了，可是真到水裡，很多人還是會發現……下一章講這個。」——它講的是**閱讀順序**，而閱讀順序已經由 `order` 欄位承載了。把它當邊會憑空造出一條「主題 A 引用主題 B」的假宣稱。
- **`l_levels`（32 個 concept 有，值是裸的 `L0`–`L6`）沒有對應實體可指。** 已把 `canonical/technica/water-sense-levels.yaml` 整檔的 id 列出來核對：**26 個 id 全部是泳式專屬**（`free.L0`、`free.L1`…、`back.L0`…、`breast.pre`、`breast.L2`…），**全庫不存在裸的 `L0` 條目**。這跟 injuries 那批的 `perception_link`「可接 L0 呼吸感知」是**同一個死路**——差別是 injuries 那三筆在 `canonical/breathing/` 找到了真正的目標，心理層這 32 筆的 `l_levels` 是**跨式的能力帶，本來就不對應任何單一泳式的 L 級條目**。
- **`diagnostic.abc_link`（值是 `A`／`B`／`C`）指向的東西沒有 `id`。** 三型的定義躺在 `water-sense-levels.yaml` 的 `three_types` 底下，A／B／C 是**三個 dict 鍵**，各自帶 `name`／`core_problem`／`manifestation`／`development`，**沒有任何一個帶 `id:`**——不是合法的邊目標。

**錯誤 34 訂正（寫完上面那段後、同一輪內查出來的，我自己前一句是錯的）：**

- **「三型只掛在 `free:` 之下」——錯。** 我當時只掃到巢狀路徑就下結論。實際上 `three_types` 是**頂層鍵**，底下四式齊全（`free`／`back`／`breast`／`fly`），每一式都有完整的 A／B／C `definition`（`back` 另有 `note`，`free`／`breast`／`fly` 另有 `quick_card`）。所以**不存在「三型只寫了自由式、與 CLAUDE.md 跨式適用矛盾」這回事**，那句推論整個作廢。**沒有 `id:` 這件事仍然成立**（四式的 A／B／C 全部沒有），結論不變，但理由只剩這一條。
- **「`l_levels` 沒有對應實體可指」——講法不準，要改。** 更準的說法是：`levels` 那 26 筆各自帶 `stroke` 與 `level` 兩個欄位，`level` 的值域正好是 `L0`–`L6` 加一個 `pre`，`stroke` 是四式。也就是說 **`L0` 在這個資料模型裡是一個「面向值（facet）」，不是一個實體**——心理層寫 `l_levels: [L0]` 並不是指著一個不存在的 ID，而是在宣告「本概念落在 L0 這個能力帶」，它會解析成四式的四筆 `*.L0`。**這不是壞掉的指標，是刻意的跨式面向。** 因此把它展開成「每個概念連四條邊」**正是錯誤 18 要擋的東西**（詞彙／面向歸屬不是邊），而且會讓 W003 憑空掉幾十筆卻沒有增加任何真實關聯。`abc_link` 同理，是跨四式的面向值。

另外直接 grep 過 `*.tech.N` 與 `<泳式>.L<n>` 兩種形態在 `psychology.yaml` 裡的出現次數：**零**。也就是說連散文點名 ID 這條後路都沒有（前幾批清孤兒靠的就是這條）。

**所以心理層的結論收斂成兩句**：① **這 70 筆的 W003 不是接線缺口**——`l_levels`／`abc_link` 是面向不是斷掉的指標，硬展開就是錯誤 18；② 真正缺的是**框架層實體**：L0–L6 這條帶與 A／B／C 三型，目前只有「四式各自的實例」，**沒有「這一帶／這一型本身是什麼」的條目**。要不要補這一層是登錄表決策（與錯誤 17 同類），**但它的正當性不來自 W003 的數字**——見下一節的使用情境檢驗。

### ✅ 使用情境檢驗的結果不是「開新實體」，是**「已寫好的 39 句公開內容，讀者一句都看不到」**（已修，已上線）

照「建新結構前先回答 Who／When／Goal」去驗，**驗出來的答案是不要建新實體**——因為真正的缺口在更前面一層。逐步查到的：

1. **`l_levels` 與 `l_note` 不在概念的頂層，而在 `public` 之下**（我前面寫「32 個 concept 有 `l_levels`」時把位置與數字都講錯了：實際是 `themes[].concepts[].public.l_levels` **37 筆**、`public.l_note` **39 筆**）。**它們是公開層內容，本來就是要給讀者看的。**
2. **`abc_link` 在 `diagnostic` 之下（22 筆）**。所以它連「該不該當公開連結」都不必討論——**照分層契約它就不能出站**，已確認 `data/vortex/psychology.yaml` 裡 `abc_link`／`reading_signals`／`probe` 全部是 **0**，剝離正確。我前面把它列為「候選邊」是第二個錯誤。
3. **`sync_vortex.py` 一直有把 `l_levels`／`l_note` 同步出站**（my-site 的 `data/vortex/psychology.yaml` 裡分別是 37／39 筆），**但全站沒有任何 layout 讀過這兩個鍵**（`grep -rn "l_levels\|abc_link\|three_types" layouts/` 回傳空）。讀頁只渲染了 theme 層的 `l_band`，概念層那 39 句一句都沒出現在畫面上。

**這才是這一章真正的缺口：不是缺連結，也不是缺內容，是已經寫好且已經同步的內容沒有被渲染。** `l_note` 是完整的解釋句（例：`psych.fear.control_loss` 的是「恐懼閘門落在 L0——在呼吸與漂浮感知建立之前就把學習擋住。」），正好承載「這個心理概念為什麼落在那一段水感發展」——也就是站主要的那條 L0–L6 脊椎。
...
**配對方式：52 筆的 `misconception` + `correct_concept` + `physical_reason` 全文讀完，逐條對照 technical-analysis 的標題判定**，沒有用 category 或關鍵字自動配。顯示字串 `cross_ref` 由目標條目的 `title` 機械生成（取「——」／「：」之前的主幹），**因此不可能出現「散文寫 A、ids 指 B」的漂移**——那正是錯誤 26–30 那一族的病根，這次從生成方式上排除掉。

**四筆跨檔指向另外說明**：`back.err7`（過度換氣）指向 `breathing.physiology.chemoreceptors`，因為正解是化學受器驅動而非技術層的任何一條；`udk.err10` 指向 `starts-turns.tech.34/44`（出發後海豚踢的正解寫在出發轉身檔）；`back.err14`／`starts-turns.err13` 另指向 `back.err7`／`udk.err8` 兩筆兄弟誤區，因為那是同一個錯誤觀念的兩種說法。四筆的 E006（`cross_ref_ids` 無法解析）實測為 0，目標都存在。

一次同時給 52 筆誤區出邊、19 筆 technical-analysis 條目入邊，共減 71。**W001 維持 1、W006 維持 0**——新寫的 52 條散文都沒有引入「文字提到某 ID 但 ids 沒列」的缺口。

### ▶️ 更早（同日）：**W003 起手：technica 兩檔 39 筆孤兒清空**（349 → **312**）——關聯本來就寫在資料裡，只是沒有寫成機器鍵

`water-sense-levels.yaml` 的 26 筆 L 級條目與 `l-indicators.yaml` 的 47 筆指標條目，是同一件事的兩面：每筆指標的 `stroke` + `level` 欄位**已經指名了它屬於哪一級**（`free.L3.evf` 的 stroke=free、level=L3）。但這個關聯只存在於兩個平行欄位裡，沒有任何一條邊。結果是 26 筆 L 級全數孤兒，47 筆指標裡有 13 筆孤兒（另外 34 筆是靠 `development/matrix.yaml` 的 `links.l_indicators` 才活著）。

做法：在每筆 L 級條目補 `links.l_indicators`，列出該級的全部指標（本式在前、`common.*` 跨式通用在後）。**這不是為了把數字做好看而造關聯**——`l_indicators` 這個鍵的宣告語意就是「→ technica/l-indicators 的 {stroke}.{level}.{aspect} ID」，關聯早就在資料裡，補的只是機器鍵。一次同時給 24 筆 L 級出邊、47 筆指標入邊。

**`pre` 的對應不靠猜，靠條目自己的內容判定：** `l-indicators` 的 level 詞彙是 `pre`／L2–L6，而 `water-sense-levels` 對自由式與仰式另用 L0／L1 表達 pre 之前的兩層，兩邊對不上。判定依據寫在各筆自己的欄位裡——`back.pre.orientation` 的 `framework_state` 就寫著「L1 仰漂感知建立」，`free.pre.resistance` 講的是靜止漂浮鬆緊與推蹬滑行距離（那是 L1 漂浮與阻力感受，不是 L0 呼吸感知）。蛙式與蝶式本來就有 `breast.pre`／`fly.pre` 條目，直接對上。

**剩 `free.L0` 與 `back.L0` 兩筆仍是孤兒，這是真的內容缺口，不補假邊：** `l-indicators` 的整套 level 詞彙從 `pre` 起跳，最低一層對到的是 L1，**L0（呼吸感知建立）在指標層完全沒有對應條目**。要接上必須先寫 L0 的指標（例如「能說出吸吐氣時身體任何變化」這類已經寫在 `water-sense-levels` 的 `indicators` 裡、卻沒進 `l-indicators` 的東西），那是新增內容不是補連結，留給後面處理。

### **W011 歸零**：63 → 0。但 W011 從來不是「缺一個欄位」，是**標錯確定性**——寫 `observation_basis` 的動作把混在推導文字裡的無主經驗主張逼出來。WARN 416 → **353**、0 ERROR、235 tests OK

W011 是「certainty 標 🟠（教練觀測）但沒有 `observation_basis`」。這個檢查是**區塊局部**的：祖先有來源不繼承到這裡，同一條目的兄弟 `evidence[]` 有來源也不算。所以每一筆有三個合法出口——寫觀察基礎、補區塊自身的來源、或**改掉標記**。

**63 筆逐條看完後的結論：這些區塊絕大多數根本不是教練觀測。** 欄位名稱本身就寫著「機制」（`mechanism`）與「物理原因」（`physical_reason`），內容主體是流體力學與解剖推導——那是 🔵，不需要任何來源。真正的問題藏在推導文字**中間**：每隔幾句就有一個關於菁英選手的經驗主張（某某入水深度 15–25 cm、前進分量降到 70–85%、直臂能量代價是彎臂四倍），沒有觀察者、沒有樣本、沒有來源，被前後的推導語氣掩護著。**寫 `observation_basis` 的價值就是強迫這道接縫顯形**——你必須說出「誰觀察的」，才會發現那句話根本沒有人在背後。

處理方式因此分三路：**改標記為 🔵**（純推導，20 筆）、**寫 `observation_basis` 並在裡面圈出不可引用的部分**（點名觀察者，多數是 The Race Club 或泳書體系，並非本專案自己的觀測；34 筆）、**補登錄來源並改標記**（內容其實是文獻的，4 筆）。

新登錄 3 筆來源（579 → **582**），另救回 1 筆佔位來源。**四筆全部是「本來就有的真論文，只是從來沒人去追」**——與 W002 的教訓同形：第一步永遠是回查，不是假設它是債。

| 來源 | 原狀 | 實查結果 |
|---|---|---|
| `src.sports-biomechanics-2026` | 佔位（display 只有「Sports Biomechanics 2026」，標 unverified） | 正是專案 CLAUDE.md 點名的可疑引用形狀（年份 2026+、無識別碼），實查後**是真論文**：Takeda 等，PMID 41728809 |
| `src.liu-2025-core-stability-youth-swimmers` | 不存在 | `free.tech.33` 早就寫著「青年泳者統合結果顯示…」卻沒有任何來源；實查逐字對得上 Liu 2025 的次群組分析 |
| `src.difernand-2025-french-swimmer-dropout` | 不存在 | PMID 與 n=160,861 早就寫在正文裡，沒有來源欄位 |
| `src.monteiro-2017-swimming-dropout-review` | 不存在 | 同上；而且一查就發現掛錯結論（見錯誤 32） |

#### 錯誤 29：`starts-turns.tech.36` 寫「旋轉半徑大（腿未收緊）增加角動量」 — **錯**

角動量在旋轉過程中是守恆的，收腿不會改變它。收腿改變的是**轉動慣量**——腿收緊使轉動慣量下降，在同樣的角動量下角速度上升、旋轉變快。原文把兩個量寫反了，而這條的實務結論（收腿讓翻滾更快）恰好正確，所以錯誤不會從結論被察覺。已改寫為轉動慣量並在原句留下更正註記。

#### 錯誤 30：心理層兩處把「ERIC 教學實踐指引」當來源 — **錯**（ERIC 是資料庫，不是發行者）

`psych.fear.control_loss` 與 `psych.fear.safety_precondition` 的「先建立信任再入水」介入都標注來自「ERIC 教學實踐指引」。ERIC 是美國教育部的教育文獻資料庫，它不發行指引——這個字串無法定位到任何具體文件。這是與錯誤 26–28 同一個病灶的第四次出現：**歸因寫在顯示字串裡，字串本身沒有可解析的目標，而且不會觸發任何檢查**。已把該字串從 `how_to` 移除，改在 `observation_basis` 裡明說這是教學界通行做法、來源不可回溯。

#### 錯誤 31：`psych.motivation.population_engines` 寫「表現最差下 10% 退出率高達 78–91%」 — **漏標性別**

Difernand 2025 原文：後 10% 群體的退出率，**女性**由 13 歲 78.1% 升到 17 歲 91.7%，**男性**同期為 69.3% 升到 83.4%。原文引用的 78–91% 是女性數值，卻寫成不分性別——而這篇研究的核心發現之一正是女性退出率一貫高於男性，抹掉性別等於抹掉結論。已改寫為兩組並列。

#### 錯誤 32：同條目寫「青少年自主支持的訓練情境比訓練量更能預測續留（系統回顧 PMID 29910410）」 — **該回顧沒有做這個比較**

PMID 29910410 是 Monteiro 等 2017 的游泳退出系統性回顧（僅完整回顧 6 篇、檢索止於 2015）。它報告的是退出主因清單——與教練衝突、有其他事要做、勝任感未見進步、來自父母或教練的壓力、缺乏樂趣、感到無聊——**沒有任何「自主支持 vs 訓練量」的預測力比較**。這是把一個聽起來合理的結論掛到一個真實但不對應的 PMID 上，比純幻覺更難抓：識別碼查得到、論文真的存在、主題也相關。已改寫為該回顧的實際內容，並在來源 notes 記下這個比較須另尋出處。

---

### **W002 歸零**：37 → 0，全庫顯示字串全部接上機器鍵。但真正的產出是**十三筆歸因錯誤**——這批工作的性質是校正，不是補登錄。WARN 453 → 416、0 ERROR、235 tests OK
...
`efetch` PMID 33641596／26839604／41076906 拿到摘要逐字比對。**結論：一筆都沒能升 `ready`**（相位專項量測的重複驗證要求沒被滿足，W020 (b) 正確擋住），但比對過程翻出兩處錯誤——**兩處都不是漏寫，是寫錯了還讀起來很合理**。

#### 錯誤 24：`free.pull.upper-trunk-rotation-hand-velocity` 這筆**拉水相**記錄裡，同時放著拉水相和推水相的數字 — **錯**

原 `value` 寫「28% 與 19%（原文報告的兩個貢獻比例；本專案未轉錄兩數各自對應的條件）」。抓到原文才知道那不是「同一件事的兩個數」：**28% 是拉水相、19% 是推水相**，相位不同；而原文第三個數字——回臂相也是 19%——量的是**前向**速度，連終點方向都跟前兩個相反。

一筆 `phase: pull` 的記錄沒有理由收著推水相的數字。已改成只留 `28%（拉水相）`，`quantity` 補上**後向**，`extrapolation_boundary` 寫明另兩個數不得搬進本筆。**三個數字裡有兩個是 19%，混用不會被任何檢查發現**——這是為什麼邊界要寫成散文而不是只靠欄位。

#### 錯誤 25：把**體節旋轉**的功率貢獻百分比，寫成**關節動作**的貢獻 — **錯**（同一個誤標在兩個檔案裡）

Kudo 等人的功率分解，單位是「手部受力 × 各**體節旋轉**所誘發的手部速度」的內積。所以 32%／37%／31% 是**胸廓、上臂、前臂三個體節**的貢獻。canonical 兩處都把它讀成關節動作：

| 位置 | 原文寫法 | 問題 |
|---|---|---|
| `stroke-demands.yaml` `free.push.three-component-power-contribution` | 數字直接對到 `action_ids` 的胸廓旋轉／肩內收／肘伸展 | 上臂體節旋轉 ≠ 肩內收，前臂體節旋轉 ≠ 肘伸展 |
| `technical-analysis.yaml` `free.tech.16`（標題／summary／mechanism／evidence 四處） | 「32%、83%、−15%，**以肩內收為主**」「32%、37%、31%，**由胸廓旋轉、肩內收與肘伸**共同產力」 | 同上，且拉水相「肩內收主導」是本專案從 83% 自行推的，原文沒這句 |

**麻煩的地方是原文確實有一句關節層級的話**（推水相中胸廓滾轉、肩內收與肘伸展同等重要）——所以誤標讀起來完全順，而且「剛好」跟數字對得上。但那句是定性描述，**不是這三個數字的出處**。兩處都已改：數字標明體節層級，原文那句定性描述另行標註，並寫明兩者不得互相代換。同時記下拉水相是 32%／83%／**−15%**（負值），形狀跟推水相完全不同，換過去會把「三者接近」的結論整個顛倒。

**這正是 `action_reference_frame`／W018 存在要防的錯誤類型**，但它們防的是欄位層級的參考系混用，防不了散文裡的層級滑動。目前沒有機器閘能擋——只能靠「引用數字前回去看分解單位是什麼」。

**順帶把兩筆來源升級**：`src.kudo-2021`（PMID 33641596）與 `src.cortesi-gatta-2015`（PMID 26839604）原本 `unverified` 的理由都是「完整作者清單未逐字轉錄」，efetch 一次補完，兩筆轉 `verified`（`src.kudo-2021` 作者姓氏 Mastuda 依原文刊載拼法保留，不擅自更正）。

**試點的結論**：查文獻這條路走得通，但**產出不是升級，是校正**。66 筆 demand 逐筆抓原文的預期收穫應該按「找出誤讀」而不是「推進 `action_status`」來估——後者受制於重複驗證，多數筆數在可見未來都會停在 `provisional`。
...
| source_id | 這次查到什麼 | 處置 |
|---|---|---|
| `src.swimmers-knee-epidemiology-sr` | `display` 只有一句中文類別描述「游泳膝傷流病系統綜述」，**無作者、無年份、無識別碼** | → **`retracted` 墓碑**。它不是「查不到的文獻」，是**從來沒有指涉對象的登錄** |
| `src.gonjo-2018` | efetch PMID 30694108 核對：`title`／`authors`／`year`／`container` **全部正確**，錯的只有 `display` 寫「Gonjo et al. 2018」 | → **只補正 `display`＋`notes`**。不轉墓碑（真文獻）、不改 ID（穩定鍵不是事實宣稱，且無條目引用它） |
| `src.lee-2008` | 抓到逐字摘要，**不能接**（見下） | 留 W008，等登錄層決策 |
| `src.liu-2025-core-meta` | PMID 41219810，青少年泳者核心訓練 SR+MA，metadata 完整無誤 | 留 W008，等 periodization／dryland 主題有條目時掛上 |

**`src.swimmers-knee-epidemiology-sr`：沒有拿「前人搜不到」當結論。** `breaststrokers-knee.yaml:89` 原本寫著「至今未能定位到實際文獻…不得引用其存在」，本次仍照專案規則重查一輪：PubMed esearch 三組關鍵字 → 候選 PMID 41342747／33010194／21893484／35291633／30787648／27409075 逐篇讀摘要。其中 **PMID 41342747**（Li & Liu, *J Sports Med Phys Fitness* 2026;66(1):82-91，45 年游泳肌骨傷害 SR+MA，n=10,973，**膝傷見於 34/94 篇＝36.1%**）確實是一篇涵蓋膝傷的游泳流病系統綜述。**但我沒有把它掛上去**——佔位字串沒有任何欄位能與它比對，掛上去等於憑空造出歸因（`CLAUDE.md`：人名歸因是高風險幻覺區）。這篇的資訊寫進墓碑 notes 保存，日後真要用**以其真實身分登錄新 slug**，不得回填墓碑。

**`src.gonjo-2018` 是活性風險，不是紀錄瑕疵。** 顯示層寫「Gonjo et al. 2018」而實體是 Nicol/Ball/Tor 2021 — 任何人讀顯示字串都會把這篇引成 Gonjo 2018，**錯誤歸因會從這一格擴散出去**。這類 `display` 與 `authors`／`year` 互相矛盾的登錄應該被當成 bug 修，而不是等有人來引用時才發現。

#### 錯誤 23：把 `src.lee-2008` 當成「找個談 body roll 的主張接上就好」的寫作任務 — **錯**

上一段那張表我自己寫的處置是「找 technical-analysis 裡談 body roll 角度而缺來源的主張接上」。抓到逐字摘要後這句站不住：

> Lee JB, Mellifont RB, Winstanley J, Burkett B. (2008). *Body roll in simulated freestyle swimming.* Int J Sports Med 29(7):569-573. PMID 18027307
> 11 名精英自由式泳者，**custom built swim bench（陸上台架，不是游泳）**；頭／胸／髖三維運動學。換氣划手：頭 roll 出現在**推進期的 30%（男）／28%（女）**，胸 42%／43%，髖 43%／40%。不換氣時序為髖→胸→頭（兩性皆同）；換氣時頭**先於**髖與胸。不換氣時男性髖 roll 遠早於女性；女性髖與胸幾乎同時。最大 roll 角度隨性別與換氣與否而異。

**接不上去的是登錄層，不是寫作。** 它的端點是「佔推進期的百分比」，這是**台架自訂的相位建構**，在 `_taxonomy.yaml#phase_model` 裡沒有對應登錄值（現有五個：`descriptive`／`race-club-6phase`／`kudo-power-phase`／`event-window`／`heinlein-2010-phases`）。硬掛到現有那筆 `descriptive` + `phase: entry` 的 `movement.demand.free.entry.shoulder-line-roll-peak` 上，等於把台架的百分比讀成入水期的角度——taxonomy 明文禁止的跨模型換算（「三者的界線定義各不相同，不得互相換算或並列比先後」）。

**這與交接單 B 項（udk 上踢→下踢交界要不要開登錄鍵）是同一類決策**，都是「要不要為一個新的相位建構開受控詞彙值」，不是可以順手寫掉的內容任務。**這也解釋了為什麼動作圖譜試點的 9 顆種子有 8 顆找得到家、剩這一顆找不到**——不是素材不夠，是相位模型的登錄粒度擋住了。

---

### ▶️ 上一輪：W008 孤兒來源 **18 → 4**：14 筆是故意不該有人引用的墓碑，改用機器鍵 `verification_status: retracted` 表示，並新增 **E013** 守住這個 fail-open 欄位。WARN 468 → **454**，0 ERROR、226 tests OK
...
**⚠⚠ 2026-09-04 再更新（`action_status` 軸已開工並走完，見本檔最上方「錯誤 33」）：下面 A 項所寫的「51 筆／74 筆」與「這是查文獻的工作」已被實作結果取代，A 項保留供追溯，不要照著開工。** 真正的候選是 **4 筆**（W020 的 (a)(b) 兩個必要條件在現有資料裡互斥，交集為 0），四筆都已逐筆查完文獻、**0 筆可升**，其中一筆（udk 仰躺上踢）已查證到「該文獻學界明文說不存在」而封閉。**下一步改做 psychology 70 筆的內容軸**——那是本階段唯一還有大量實料可推進的地方。**但先讀本檔最上方的錯誤 34 再動手**：心理層整檔沒有 `links:`／`cross_ref_ids:`，且檔內三種看似關聯的欄位（`bridge_zh` 敘事轉場、`l_levels` 裸 L 級、`diagnostic.abc_link` A／B／C）**全部沒有合法目標**（全庫 26 個 L 級 id 都是泳式專屬、三型是無 `id` 的 dict 鍵）。**這一章的第一步是登錄表決策——要不要開泳式無關的 L 能力帶與 A／B／C 三型實體——不是接線，也不是補欄位。** 直接照字面去把 `l_levels` 轉成邊會同時踩到錯誤 18（詞彙歸屬不是邊）與「不要為了數字造關聯」。
...
- **C. WARN 債務：W003 126（孤兒條目）、W008 3（孤兒來源），合計 129；W001／W002／W011 均已歸零。** W011 於 2026-09-04 走完 63 → 42 → 29 → **0**（教學誤區 21 筆、技術分析 33 筆、心理 6 筆、L 指標 2 筆、感知協議 1 筆）；產出是四筆錯誤（錯誤 29–32）與四筆來源補登錄，不是補欄位。 W008 已於 2026-09-04 收尾至終端狀態——**剩的 3 筆是真實已驗證文獻等著被掛上主張，不該用「消警告」的方式歸零**（硬接會製造錯誤歸因）；其中 `src.lee-2008` 卡在相位模型登錄粒度（見本檔最上方錯誤 23）。 W002 於 2026-09-04 三輪走完 123 → 37 → **0**：前 86 筆是識別碼比對的機械遷移，**最後 37 筆裡有 8 筆同樣不缺來源、只是漏連機器鍵**（來源早就登錄在同一條目的別處欄位），真正需要查文獻的不到三分之一——把 W002 當成「缺來源、要去找文獻」來估工會高估數倍，第一步永遠是拿識別碼回查 registry。這輪的實際產出是**十三筆歸因錯誤**（錯誤 26–28，見本檔最上方），不是新登錄。除 W003 外全部先於本輪存在。**W003 走了六步：392 →（錯誤 16，補 `cross_ref_ids` 入邊）347 →（錯誤 18，拿掉散文與詞彙兩類假邊）403 →（錯誤 19，出入邊改對稱）399 →（starts-turns 三筆 injuries 接上 `technical_link_ids`）397 →（Class ② 33 筆章節號 → `cross_ref_ids`）349 →（technica 兩檔補 `links.l_indicators`，L 級 24 筆出邊＋指標 13 筆入邊）312 →（teaching-errors 52 筆孤兒誤區逐條配 `cross_ref_ids`，誤區 52 筆出邊＋技術分析 19 筆入邊）241 →（breathing 全域 16 筆出邊＋2 筆 `links.drills`，20 筆脫離孤兒；perception `public_layer_pointers` 自造鍵改機器鍵）220 →（periodization 14 筆出邊，第二輪改用 YAML 鍵名重掃才補齊）198 →（technical-analysis 42 筆標題級點名出邊＋3 處 `evidence_from`／2 處 `cross_ref` 散文機讀鍵補收，46 筆脫離孤兒）198 → 152 →（injuries 14 筆條列項點名出邊，含跨 domain 接 `canonical/breathing/`；另 7 筆 categories 詞彙表由 `id`／`zh` 改 `key`／`name_zh`，不再被 `iter_entries()` 誤判成條目）**126**。下表分類已於 2026-09-04 重跑，以 W003=349 為準（分類方法：對每一筆孤兒實際打開 YAML 看它有哪些關聯欄位；腳本存在 `tools/classify_w003_orphans.py`）。
  ...
  **已接第一批（2026-09-03）：`health/drafts/` 三筆 starts-turns 傷害的 `technical_link_ids`。** 接法是先讀 `links.technical_link` 那句人讀散文說了什麼，再去 `technical-analysis.yaml` 找**那句話點名的機制**，不是找標題像的條目：
  ...
  - `flip-turn-wall-push` → `starts-turns.tech.38`（翻滾啟動時序太早／太晚）＋ `tech.21`（觸牆膝屈曲 100–120°，tuck 過深過淺）＋ `tech.22`（腳觸牆深度決定推蹬方向，r = 0.76）。散文寫的是「時序與足位精準度」，這三筆正好是時序一筆、足位兩筆。傷害條目引的 `src.pmc6409673-c` 與 `tech.21` 引的 `src.pmc6409673-b` 是同一份研究。
  - **W003 只掉 2 不是 3**：三筆都離開孤兒名單了（已 grep 確認），代表其中一筆本來就有入邊、不是靠這次才脫離。數字對不上時先問哪一筆本來就不在名單裡，不要先假設漏改。
  - `indices/gap_report.json` 的 `unlinked_records` 同步為 **397**，與 W003 相等（錯誤 19 之後兩者共用同一份邊定義，對不上就是又分岔了）。
...
- **E. ✅ my-site 已同步並實測上線（`a089e38`，2026-09-03）。** `vortex-movement.html` 的硬編數字已是 **59/59**（六式分列），第四、第五次推翻的自我更正段落已補上，末段收在「空白理由有兩型、兩型都不是資料問題」。**線上實測**：`curl` Pages 頁面命中 `59 / 59`，Deploy Hugo workflow `success`，工作區與 `origin/hugo-source` 同步。
  - 蛙式腿外划那段是用戶裁決的落地形狀——**不裁決路徑往外／關節在伸展誰對，兩個都寫，改去交代辨識順序**（先看腳掌方向有沒有在膝髖開始伸展前轉好，再看有沒有維持住），並明寫不要拿「蹬得多快多用力」當判準。往後遇到同型的內在衝突照這個處理。
