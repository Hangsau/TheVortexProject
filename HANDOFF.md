...
### ▶️ 兩個相位補上。覆蓋 54/59 → **56/59**，需求 61 → **63**。**「缺量測」這個空白理由連續被推翻兩次，兩次都是缺查找（錯誤 11）**

| 產出 | commit | 內容 |
|---|---|---|
| `starts-turns.breaststroke-pullout` 新需求一筆 | `6157237` | 三種變體差在順序不差在關節；胸椎與腰椎在同一時刻朝相反方向 |
| `udk.down-to-up-transition` 新需求一筆 | 見下方 commit | 可辨識的是髖有沒有拱起、腳有沒有掛下來 |
...
**驗證**：0 ERROR、WARN 604 → 602 → **601**、W019 維持 0、**211 passed／8 subtests**、`KNOWLEDGE_MAP.md` 已重生成。
...
**仍然缺的 3 個相位**（`breaststroke-pullout` 與 `down-to-up-transition` 都已於本日稍後補上，兩次的空白理由都是錯的，見最上方）：

| 相位 | 類別 | 下一步 |
|---|---|---|
| `starts-turns.underwater-dolphin-kick` | **待寫，不待測**（理由已於 2026-09-03 更正） | 素材已找到：`src.hall-murphy-2020` Ch 22 第 150–174 行比較**俯臥／側臥／仰臥**三種身體位置（實測平均速度 1.56／1.64／1.70 m/s），第 188 行給關節組成。切入點是**同一組關節動作在三種身體位置下對應到完全不同的池畔方向**——`action_reference_frame` 正是為此存在。側臥是既有記錄完全沒碰過的第三種位置（現有兩筆只有俯臥與仰臥） |
| `udk.kick-initiation` | **這一格該寫什麼尚未定案**（不是量測缺口） | `udk.tech.17`／`tech.29` 的獨有內容是**起踢時機**（速度交叉點，群體起點 1.9–2.2 m/s），自變項是速度不是關節；而它的關節組成會與 `down-kick` 重複。可寫的角度是 Ch 22 第 188 行那句「全程頭與手臂維持超流線」——**起踢這一刻唯一不該變的是肩與頸，會變的只有髖膝踝**。是否足以獨立成筆需裁決 |
...
### 最高優先——先把 `starts-turns.underwater-dolphin-kick` 寫掉；它不缺量測，缺的是動筆

**先前列的四個「待決事項」現在全部處理完**（2026-09-03 三次更新後重寫，舊清單保留在下方僅供追溯）：
...
| c 之「補量測」半 | ⏳ **從四格降為兩格，而且兩格都不是量測問題**。`breaststroke-pullout`（錯誤 11）與 `udk.down-to-up-transition`（錯誤 12）都是翻書就有，兩次的空白理由都是同一種錯 |
| c 之 `underwater-dolphin-kick` 分母 | ✅ **已決：分母維持 59，除名提案撤回**（見錯誤 10） |

**「三個相位缺關節層量測」這句話現在只剩零個成立。** 先前這裡寫著「一次水下側視高速拍攝可全數回答」，並列了貼標記、≥100 fps、三種下水條件的拍攝協議——**那份協議可以整段刪除**，因為它要回答的三個問題裡，兩個翻書就有答案，第三個根本不是量測問題：

| 相位 | 現在的實際狀態 | 該做什麼 |
|---|---|---|
| `udk.down-to-up-transition` | ✅ **已寫**（2026-09-03，見最上方）。`src.hall-murphy-2020` Ch 22 第 188 行整段用踝蹠屈／膝屈伸／髖屈曲／腰椎伸展／胸椎屈曲寫海豚踢 | 無 |
| `starts-turns.underwater-dolphin-kick` | **待寫，不待測**。素材同一章第 150–174 行：俯臥／側臥／仰臥三種身體位置的實測平均速度 1.56／1.64／1.70 m/s，第 194 行給各段肌群 | **下一件事就是寫它**。側臥是既有 63 筆記錄完全沒碰過的身體位置，`body_position` 目前只有 `prone`／`supine` 兩值在用，寫這筆要先決定第三值怎麼登錄（此欄未受驗證器管，但不可隨手造字） |
| `udk.kick-initiation` | **這一格該寫什麼尚未定案**，不是缺量測 | 要先決定範圍。它的獨有內容是**起踢時機**（速度交叉點，群體起點 1.9–2.2 m/s），自變項是速度不是關節；若只寫關節組成會與 `down-kick` 重複。唯一可能的獨有關節內容是 Ch 22 第 188 行的「肩頸維持超流線不動、動的只有髖膝踝」——**這是不是足以獨立成一筆，是撰寫範圍的判斷，不需要任何新資料** |

**判別法（本專案已用三次，後兩次都用錯）**：問「這個相位裡有哪些關節在動」——素材答得出關節名就是問法或**查找**的問題，只答得出距離、速度、時機、策略才是量測問題。**錯誤 11、12 顯示這條要配一個強制前置動作才有用：判定「答不出關節名」之前，必須實質檢索 `resources/books/` 一次，不能只憑既有 canonical 素材下判斷。** 同一個錯誤在同一天被抓到兩次，代表它不是偶發——**往後任何一格要標成量測缺口之前，先翻書，並在該格的理由欄寫明翻過哪一本哪一章。**

**可推廣的一條**：**當一項待辦被標成「需要外部資料才能定案」時，先問這筆內容是否真的需要那個精度、以及那個資料是否其實已經在手上。** a 卡在「要幾度」、b 卡在「哪個方向對」、flip 卡在「要不要換框架」、pullout 與 down-to-up-transition 卡在「以為要新量測」——五者的共同結構是**把可寫的內容誤設成待取得的資料**。五個都在改問法或翻書之後當場可寫。
