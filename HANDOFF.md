...
- Talos 協作模式討論：Drive 通道繼續用，但任務設計要升級——給任務時附上 `_INDEX.md` + 明確指出「不要重複哪份文件」，中期再考慮給他確定性標記規範

### 感知學習——重點摘要

- **Gibson 分化理論**：感知學習 = 學會辨別差異，而非加入新東西；每個 L 級轉換對應一個分化完成
- **皮質重組**：S1 觸覺訓練後皮質重組，支撐水感訓練有效的神經基礎
- **注意力特異性**：注意力指向決定什麼被學習；形式監控佔用注意力時，水感學習被抑制（核心機制）
- **陸地遷移限制**：本體感覺可遷移，水動力感知無法從陸地建立
- 與因果倒置假設的整合：「注意力決定建什麼地圖」有了神經科學機制說明
- 🔴×3（觸覺跨部位遷移、水中感知學習直接研究、閾下學習角色）

### 下一優先選項

**A. Research 層繼續擴充（最自然的下一步）**
...
**C. Instructional 層補完**
- `出發與轉身教學誤區深探.md` 目前是「骨架初版」，🔴×4 待深探
- 可與感知科學層完成後一起做，會有更好的感知框架支撐
...
*最後更新：2026-04-18（文件整理：RESEARCH_PLAN / _INDEX / HANDOFF / FUTURE_RESEARCH 全部同步至當前狀態；2.1渦流回收標記已畢業；4.1感知目標語言標記完成）*
