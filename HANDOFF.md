...
### 進行中——四式關節動作筆記的教科書驗證（P1 完成，P2 未動）
...
1. **P2（下一窗）**：A 類 59 條逐條對 Neumann／Nordin 定位裁決，附章節。依泳式串行分批，每式完成立即 commit。
2. **P4**：C 類 17 條轉 `swimming-kinetic-chain` 文獻集；查不到者明標「待游泳文獻」，不得偽裝已驗證。
3. **P5**：裁決結果併入既有 `Instructional/自由式深度技術分析.md`、`仰式深度技術分析.md`（**修現有條目，不另開檔**）；完成後依 `resources/CLAUDE.md` 刪除 `raw/notes/` 原檔。
4. **蛙式／蝶式／超流線（餘 3 式）**：待 Claude 7D reset（08-21）後再拆，本窗刻意不做——7D 剩 33% 撐不起全部 177 項判定。
5. **B 類 5 條 ROM 查表**：待 codex 恢復（08-21 14:10）發包。MiniMax **不可用於此**（專用於經文）。
