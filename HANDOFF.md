...
### ▶️ **W011 歸零**：63 → 0。但 W011 從來不是「缺一個欄位」，是**標錯確定性**——寫 `observation_basis` 的動作把混在推導文字裡的無主經驗主張逼出來。WARN 416 → **353**、0 ERROR、235 tests OK
...
`gap_report.json` 的 `unlinked_records` 349 與 W003 對齊，`unused_taxonomy_values` 維持 7。剩餘 353 WARN 全是內容工作，不是登錄債：**W003 349**（孤兒記錄，內部連結問題）、**W008 3**（不該歸零，`src.lee-2008` 卡在相位模型的登錄層決策＝錯誤 23）、**W001 1**。W011 已於同日歸零（見下）。
