...
**最近一次 session 完成了 A（UDK 🔴 查證）與 B（UDK Bridge）。**

**下一優先：出發轉身 🔴×5 查證**

五個待查項目難度不同：
- **容易查**：蛙式觸牆策略（有明確教練觀測文獻）、背→蛙規則（FINA 規則文本可查）
- **中等**：觸牆膝蓋角度（有生物力學研究，需確認具體數字來源）
- **較難**：翻滾啟動距離、仰式計步（個體差異大，文獻少）

建議策略：先從容易查的開始，能補就補，補不了留 🔴 說明。
