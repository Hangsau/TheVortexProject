...
1. **B 類 8 條 ROM 查表**（自由式／仰式 5 條＋本批 BR-24、BF-15、ST-01）：待 codex 恢復（08-21 14:10）發包，查兩本教科書的正常 ROM 上限。**MiniMax 不可用於此**（目前專用於經文 pipeline）。
2. **C 類 39 條**：轉 `resources/books/swimming-kinetic-chain/` 文獻集處理。其中 **6 條 ⚠︎⚠︎ 高風險傷害宣稱最優先**（BR-44/45 蛙式肩前脫位、BR-46/47 蛙式膝、BF-06 蝶式夾擠位置）——原文的 cite index 格式不可解析，**追溯到 PubMed 原文前不得寫入任何公開層**。查無文獻者明標「待游泳文獻」，不得偽裝成已驗證。
3. **P5 第二階段（可選，優先序最低）**：把 12 條 canonical 發現的教學語言回寫進 `Instructional/*深度技術分析.md` 對應章節（**改既有檔，不另開新檔**）。canonical 已落地，散文層只是呈現層。
4. **收尾**：上述完成後依 `resources/CLAUDE.md` 刪除 `raw/notes/游泳四式關節動作全拆解(整合版).md` 原檔。
