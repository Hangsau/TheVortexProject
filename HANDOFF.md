...
| `udk.kick-initiation` 新需求一筆 | 見下方 commit | 可辨識的是第一踢的膝彎多大，不是「什麼時候踢」 |
| `breast.leg-outsweep` 新需求一筆 | 見下方 commit | 相位名稱是「外划」，關節內容卻是「伸展」——兩件事都真，本層不裁決 |
...
**驗證**：0 ERROR、WARN 604 → 602 → 601 → 599 → **597**（W003 孤兒條目逐次遞減，每筆新紀錄的 `cross_ref_ids` 都接回原孤兒）、W019 維持 0、**212 passed／8 subtests**、`KNOWLEDGE_MAP.md` 已重生成。**驗證器本身也改了一處**：`cross_ref` 契約檢查擴到 `diagnostic` 層（原本只跑 entry 頂層與 `public`），補上 6 個區塊的零覆蓋；擴完 WARN／ERROR 皆不變，代表是補洞不是清債。
...
- **C. WARN 債務未動：W002 123（顯示字串未遷 `source_ids`）、W003 392（孤兒條目）、W011 63（🟠 缺 `observation_basis`）、W008 18、W001 1。** 這 597 筆全部先於本輪存在，補相位覆蓋的過程只讓 W003 遞減。**W003 是其中唯一會隨新記錄自然下降的**（每筆新記錄的 `cross_ref_ids` 接回原孤兒）；另外三類要專門處理。**動它們之前先確認 W003 的孤兒定義是否對「只被 diagnostic 層引用」的條目誤判**——`cross_ref` 契約本輪才擴到 diagnostic 層（`7af381e`），孤兒計算是否同步擴過還沒查。

- **D. 測試 harness 的複製迴圈是結構性風險，本輪只補了一處。** `tests/test_validate.py` 自己重寫了 `validate.py` 主迴圈而不是呼叫它；本輪加 `diagnostic` 層時產品端改對了、測試端沒跟上，結果是**新層在測試裡零覆蓋而全部測試仍然綠燈**。已在該處加註解釘住，但**同一個檔裡可能還有別的複製迴圈**，值得掃一遍。這是「commit + tests pass ≠ 實際生效」的反向版本：**改對了，但測試證明不了它改對了。**
