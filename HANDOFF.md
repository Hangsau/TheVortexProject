...
- **`diagnostic.abc_link`（值是 `A`／`B`／`C`）指向的東西沒有 `id`。** 三型的定義躺在 `water-sense-levels.yaml` 的 `free.three_types.definition` 底下，A／B／C 是**三個 dict 鍵**，各自帶 `name`／`core_problem`／`manifestation`／`development`，**沒有任何一個帶 `id:`**——不是合法的邊目標。另外它還掛在 `free:` 之下，而三型診斷按 `CLAUDE.md` 是跨式適用的。

另外直接 grep 過 `*.tech.N` 與 `<泳式>.L<n>` 兩種形態在 `psychology.yaml` 裡的出現次數：**零**。也就是說連散文點名 ID 這條後路都沒有（前幾批清孤兒靠的就是這條）。

**所以心理層的正解是內容工作沒錯，但它的第一步是登錄表決策，不是撰寫也不是接線**——要不要為「跨式 L 能力帶」和「A／B／C 三型」各開一組**泳式無關的實體**，讓 `l_levels` 與 `abc_link` 有合法目標可指。**這與錯誤 17（matrix 那 12 格四個 links 鍵沒有合法目標）是同一類問題**，不是新形態。在那個決策做出來之前，**這 70 筆的 W003 不會動，也不該動**——照本檔既有的告誡：不要為了讓數字好看而造關聯。
