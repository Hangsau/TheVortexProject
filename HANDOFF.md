...
**E 組之外仍掛著的一個 🔴（週期化層，非 E 組範圍）**：`set-design.yaml#anchors` 的 **tier_c 距離—時間外推係數**，本庫自己標「尚未查證」。沒有它，只有 T50 沒有 T200/T400 的泳者只能吃 tier_d 自校準。查證方向是游泳專屬的臨界速度／距離—時間關係（Wakayoshi 1992/1993 與 di Prampero 2008 已在庫內，可以從它們的參數形式往外找），不要直接套跑步的 Riegel 指數。
