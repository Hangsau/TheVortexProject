...
| `starts-turns.underwater-dolphin-kick` | 分母問題 | 與 udk 分期模型重疊，應考慮 59 → 58 而非補寫 |
...
**唯一還在的撰寫類障礙：三個相位缺關節層量測**（`udk.kick-initiation`／`udk.down-to-up-transition`／`starts-turns.breaststroke-pullout`）。與 a、b 不同，這三個**不是問法錯**：庫內既有素材講的是空間軌跡、時序與速度，沒有任何一句可對到關節詞彙。要寫必須先有相位化三維運動學。**這是研究工作不是填充工作，不可發包。**

**`underwater-dolphin-kick` 的分母問題該收掉**：它與 udk 七相分期模型完全重複，除名會把 59 → 58。分母已印在 my-site 公開頁上，改分母要連帶改公開文案。建議做法是**除名並在公開頁註明「原第 59 項與海豚腳分期重複，已併入」**，而不是為了讓數字好看去補一筆重複記錄。
