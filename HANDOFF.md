...
心理層的缺口補強任務完成（查證債 / 整合缺口 / 族群覆蓋 / 補厚單薄段）。剩餘僅兩件跨檔工程，**非心理層內部任務**：
1. **`Research/感知科學/隱性_顯性學習.md` 升 v2**：先修 escape-bug（整檔字面 `\n`），再用 dossier 02（再投資動作 vs 決策分離、MSRS、Beilock 明確監控）+ 07（Liao & Masters 2001 比喻學習、de-reinvestment）素材把 3KB stub 擴成 v2。此檔是 cluster 02/07 的天然下游。
2. **各 dossier 殘餘 🔴 逐條查證**：多屬研究空白等級（已明確標界「目前文獻無此數據」），非未完成查證債；可低優先逐條追，或保留為未來實驗設計起點。
...
*最後更新：2026-06-15（第二 session：心理層查證債清理——08 六條 🔴-P PMID 全查證 + 修三處誤歸因、06 Bauman 兩條 🔴 釐清；教學對象框架釘入 CLAUDE.md；心理層 B/C/D 缺口路線圖已記）*
