...
週期決策工具第一版已從流量中斷／重啟後的本機檔案接續完成，本機驗收通過，正進入提交與發布。上一階段資料、程式、執行紀錄均有保存；中斷点為 2026-09-10 08:05 的 `zones.yaml` 同步比對誤報。兩個 repo 的 29 個原始變更檔已另外備份在 `C:/claudehome/tmp/periodization-recovery-2026-09-10/`，有 ZIP 與逐檔 SHA-256。
...
先完成本批提交、CI 與正式站驗收並補記 SHA；my-site 含新同步 allowlist 的版本先推，canonical 後推，避免自動同步漏新檔。之後依實際使用回饋調整決策，不再從零重建。功能不儲存表單，需用「複製完整安排」保存個案結果；沒有背景研究排程。下方保留其他研究線的歷史待辦。
