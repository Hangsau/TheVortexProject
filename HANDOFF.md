...
### ▶️ 兩件卡關項由使用者裁決解開，另有兩個相位補上。覆蓋 52/59 → **54/59**，需求 59 → **61**
...
**仍然缺的 5 個相位**（分兩類，兩類的下一步不同）：

| 相位 | 類別 | 下一步 |
|---|---|---|
| `udk.kick-initiation` | 缺關節層量測 | 需相位化三維運動學，不是再查文獻 |
| `udk.down-to-up-transition` | 缺關節層量測 | 同上 |
| `starts-turns.breaststroke-pullout` | 缺關節層量測 | 同上 |
| `starts-turns.underwater-dolphin-kick` | 缺關節層量測（**本輪重新分類，見錯誤 10**） | 同上。分母維持 59，不除名 |
...
### 最高優先——三個相位缺口需要新量測，這是本圖譜第一次碰到真的天花板

**先前列的四個「待決事項」現在剩一個半**（2026-09-03 使用者裁決後重寫，舊清單保留在下方僅供追溯）：
...
| c 之「補量測」半 | ⏳ **仍然卡住，且是真的卡住** |
| c 之 `underwater-dolphin-kick` 分母 | ⏳ 仍待決 |

**唯一還在的撰寫類障礙：四個相位缺關節層量測**（`udk.kick-initiation`／`udk.down-to-up-transition`／`starts-turns.breaststroke-pullout`／`starts-turns.underwater-dolphin-kick`）。與 a、b 不同，這四個**不是問法錯**：庫內既有素材講的是空間軌跡、時序、策略與速度，沒有任何一句可對到關節詞彙。要寫必須先有相位化三維運動學。**這是研究工作不是填充工作，不可發包。**

**分母 59 維持不動，`underwater-dolphin-kick` 除名的提案撤回**（理由見上方錯誤 10：跨分期模型的平行相位不視為重複，`underwater-glide` 已是庫內先例）。**同時撤回「為了讓數字好看補一筆重複記錄」的反面警告在這一格的適用**——這一格現在的正確處置就是留白等量測，與另外三格一致。

**這一格也是本輪唯一沒有被「換問法」解開的**。a、b、flip／wall-rotation 三項都是問法錯，換問法當場可寫；這四格是真的沒有素材。**兩者的區別可以一句話判**：問「這個相位裡有哪些關節在動」，如果素材答得出關節名就是問法問題，答不出來（只答得出距離、速度、時機、策略）就是量測問題。

**從 a、b 兩項可推廣的一條**（值得寫進下次規劃的前置檢查）：**當一項待辦被標成「需要外部資料才能定案」時，先問這筆內容是否真的需要那個精度。** a 卡在「要幾度」、b 卡在「哪個方向對」、flip 卡在「要不要換框架」——三者的共同結構是**把辨識問題誤設成裁決問題**。三個都在改問法之後當場可寫。
