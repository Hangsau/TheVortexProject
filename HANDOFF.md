...
**已知未處理**：`indices/*.json` 與 `KNOWLEDGE_MAP.md` 是 canonical 這側自己產的，沒有經過 sync，所以仍帶著折行空白。影響是**跨折點的片語 grep 不到**（單詞沒事）。要修就是把同一個接合函式搬進 `build_indices.py` / `build_knowledge_map.py`；優先度低，但下次有人 grep 不到明明存在的句子時，先想到這裡。
