...
- 裁決：**修正**｜衝突型 1（座標系／術語系統未對齊），非型 5
  - 判定：原註記標型 5（真實學界分歧）不成立。分期命名不是有證據的對立主張，是各家各自的操作定義——Fernandes 2022 用四 sweep 編碼影像並外加 recovery（5 相）、Chollet 2008 與 Race Club 用 6 相、Science of Swimming Faster 用 Catch／Midpull／Finish／Rotation。三套都自洽，沒有一套是錯的，也沒有共識研究可裁。依型 1 解法：**不選邊，把差異顯性化**。
...
- 裁決：**刪除效益句 + 降為未證**｜非衝突，是三層皆無
  - 判定：這是本批唯一核心關係三層皆無者，且不是「沒查到」而是**歸因錯位**。教材把 reach 歸給 body roll（`20_Figure_2.2.md:17`），不是肩胛前伸；Tovin 2006 把 scapular protraction 放在 **pull-through**，不是前伸滑行期；Wanivenhaus 2012 的方向甚至相反——正確 body roll 是為了**減少**維持盂肱對位所需的肩胛前伸。所以連「前伸滑行期會肩胛前伸」這個相位歸屬本身都缺直接量測。
  - 可寫入：無。整條降為 `unverified`，保留原文以擋日後重犯（協議 §5：不刪錯誤版本）。
  - 不得寫入：「肩胛前伸可增加划距」；任何把肩胛動作接到 stroke length／速度／效率的句子。
  - movement 落地：**不得建立**「肩胛前伸 → 划距」的 demand 或對應 intervention。若 W4 要處理前伸滑行期的划距，來源必須改掛 body roll（教材歸因），且與肩胛條目分開。
  - 根因：**結構性**——見文末結構性根因 1（效益句自由代換）。
...
- 裁決：**部分支持 + 修正**｜衝突型 3（簡化 vs 精確）
  - 判定：主張拆成三塊，證據強度差很多。②「手與前臂作為單一單元、腕被穩住」是本批**證據最紮實**的一條：教材直接說 “a single unit, connected by a stiffened wrist”，Caty 2007 用 FCU／ECU EMG 直接觀察到 insweep 期的腕穩定與高拮抗共收縮，教材模型與一手 EMG 同向。①「伸直或輕度屈曲」的二選一無來源——教材只給對齊（中立），Caty 量的是穩定不是角度，「或輕度屈曲」是原筆記自行加寬。③「划槳面」是水動力效益語，無任何力或效率終點。
  - 可寫入：「抓水至早期推進期，腕維持穩定，手與前臂共同構成受力單元」（教材模型 + Caty 2007 EMG）。
  - 不得寫入：抓水腕角的具體選項（伸直／輕屈）；「形成划槳面」這類面積或推進效益語。
  - movement 落地：**這條可直接產出 demand**——腕屈伸肌共收縮（FCU／ECU）的穩定需求，證據層級為直接游泳 EMG，是本批最適合進 `canonical/movement/` 的一條。需求寫成「腕關節等長穩定」，不是「腕擺到某角度」。
  - 根因：個案（角度值無來源加寬）+ 結構性根因 1（效益句）。
...
- 裁決：**支持（限姿勢層）+ 修正**｜衝突型 3（簡化 vs 精確）
  - 判定：型 3 的標準案例，依解法**不判錯**，保留教學簡化版並註精確版。Suito 2017 以肩／肘／腕三維座標建 high-elbow index，明定於 pull phase 評估；原句「肘高於手掌」是同一件事的粗粒度說法，不是錯的，但也不是研究的操作定義（研究看的是三關節相對構型，不是單純垂直高度差）。Heinlein 2010 另把 early pull-through 定為 glide 結束至肱骨約 90°，可用來界定相位。
  - 可寫入：「拉水期維持高肘構型」+ 註「教學簡化語；正式定義為 Suito et al. 2017 的三關節 high-elbow index，本專案未採用其門檻值」。
  - 不得寫入：「保持高肘可提高速度／降低阻力」的因果句——Suito 是相關研究不是介入試驗；教材的 “reduce frontal drag” 是模型陳述，無量測。樣本為 22 名男性，不得外推女性與長距離。
  - movement 落地：相位歸屬用 Heinlein 的 early pull-through 邊界；demand 寫肩胛與肩袖維持肱骨位置的等長需求，不寫「高肘→快」。
  - 根因：個案（簡化語未標）。
...
- 裁決：**強制修正**｜非衝突，是證據層級冒用（負荷描述冒充流行病學）
  - 判定：協議 §1「發生率、盛行率、風險倍數、『常見／好發』等頻率語言只能來自文獻，沒有就是沒有」。本條「好發位置」正是頻率語言，三層零支撐。更關鍵的是**現有最高等級證據方向相反**：McKenzie 2023 系統性回顧（22 篇、單篇最高 n=201）對 80 個變項給出 “no strong evidence supporting or refuting”，且 specialty stroke 與肩傷關聯是 “moderate evidence opposing an association”。可保留的只有負荷描述：Tovin 2006 “increased stress on the anterior capsule”、Wanivenhaus 2012 入水時 overhead elevation + external rotation 對應前向盂肱不穩。
  - 可寫入：「仰式入水／抓水的過頭外旋位置使肩前關節囊承受較高負荷（Tovin 2006、Wanivenhaus 2012 的臨床負荷描述）」+ 明標「無此位置特異的傷害頻率或風險資料；McKenzie 2023 系統性回顧對泳式專項與肩傷的關聯給出 moderate evidence opposing」。
  - 不得寫入：「好發」「常見」「容易造成」任何頻率或因果傷害語；具名診斷。
  - movement 落地：本條**不產出 intervention**。若 W4 要接肩前側穩定訓練，`mobility_decision` 只能是 `evidence-gap`（依 W016 必須配 `do-not-prescribe` 且 `dosage_source_ids` 為空），不得因為「負荷高」就開處方。
  - 根因：**結構性**——與 `plans/關節主張裁決_C類傷害宣稱.md` 已證實 3/5 傷害宣稱不符文獻是同一模式，見文末結構性根因 3。
...
- 裁決：**修正（兩處，方向相反於原文）**｜① 屬同層來源優先序裁決，② 屬衝突型 3
  - 先排除誤判：這**不是型 2**（型 2 限同一文件自相矛盾），也**不是型 4**（型 4 是解剖教科書 vs 游泳文獻）。本條是原筆記（來源優先序第 4 層：商業教學網站）對上游泳教材（第 3 層），同層衝突依 §4 由高層裁決；且兩本教材彼此一致，第 2／3 層無一手量測可推翻。
  - ①「腕伸直」→ **改為輕度屈曲**。教材是直接否證而非查無：`37_Figure_3.1.md:7` “The first movement is to bend at the wrist”、`17_Chapter_14...md:46` “with the wrist slightly flexed”。兩本書同向，原文寫反。
  - ②「掌心朝下後方」→ **改為掌心朝後（指尖朝側），並加禁令**。這一處是型 3 而非直接矛盾：`37_Figure_3.1.md:16` 禁的是 “pitched downward to face the bottom of the pool”，而「朝下後方」的合成向量未必等於朝池底，粒度不同不判錯；但原文的方位詞模糊到會被讀成教材明確禁止的姿勢，故用教材的精確措辭取代，並保留禁令句。
  - 可寫入：「抓水起始腕輕度屈曲，指尖朝側、掌心朝後；初段掌心不可轉為朝池底」（Science of Swimming Faster Figure 3.1；Race Club Ch.14）+ 標「僅教材模型層級，無三維腕角一手量測」。
  - 不得寫入：由掌心朝向反推前臂旋前／旋後（同 BF-05／BF-25 的缺陷 3）；「划水面」的面積或推力效益語。
  - 根因：**結構性**——見文末結構性根因 6（腕位／掌向憑解剖中立直覺填空，未查教材）。本條與 BF-05、BF-25 三條同錯。
...
- 裁決：**部分支持 + 拆分標記**｜衝突型 3（相位名稱與操作邊界的粒度差）
  - 判定：兩個子句證據等級不同，必須拆開標，不能一起收。
  - ①「手掌路徑再度向下」：支持（教材層）。`39_Figure_3.3.md:17` 與 `17_Chapter_14...md:128` 都把向下動作放在 finish。但要註明一個粒度落差：Fernandes 2022 的相位操作定義是 “from the hand below the shoulder to the end of its **backward** movement”——相位名叫 down sweep，邊界卻以向後移動終止定義。這是型 3，**不判錯**，但引用時不得把研究的相位邊界當成「向下」的量測證據。
  - ②「肩持續伸展」：三層皆無同步肩角量測。依協議 §1 可走解剖推導路徑——仰臥姿下手由肩下方往腳側移動對應盂肱伸展持續——但必須引 Neumann 的關節骨動學段落當依據，且條目內明寫「此為解剖推導，尚無游泳專項文獻證實」，標 🔵 不得標 🟢。
  - 可寫入：①如上；②以 🔵 推導形式寫，並附推導依據出處。
  - 不得寫入：把②寫成有運動學支撐；把 Fernandes 的相位定義當作肩角證據。
  - 根因：個案。
...
- 裁決：**支持（技術描述層）+ 強制加術語警示**｜衝突型 1（術語撞名），**非型 2**
  - 先排除誤判：蒐證階段標記教材「把 late rotation 標為技術錯誤」與本主張對撞，這是**誤讀，不成立**。逐句比對：教材 `40_Figure_3.4.md:25` “The rotation should happen only at the beginning and end of the arm strokes, **not during the most propulsive parts**” 與續句「一手入水、另一手 finish 時快速轉」——這與本主張「拉水過程不轉、推水末段快速轉」**是同一件事，方向一致**。`:31` 的 “Late rotation” 指的是整體旋轉遲到、導致對側抓水銜接不良的錯誤徵象，與本主張的「推進段維持不轉」不是同一個所指。這正是協議 §3 警告的「把回答不同問題的東西當矛盾去裁決」，若照原判會砍掉一條與教材一致的正確內容。
  - 可寫入：「轉體發生在入水與推水末段的銜接處，主要推進段維持不轉」（Science of Swimming Faster Figure 3.4；Race Club Ch.16）。**措辭必須是「推進段維持不轉」，不得單獨使用「延遲旋轉」四字**——那會與教材的 late rotation 錯誤徵象撞名，讀者無法分辨。條目需並記兩個所指的區別（型 1 解法：差異顯性化）。
  - 不得寫入：把「延遲旋轉」寫成可操作的介入或訓練指令——Gonjo 2021 只量到 roll 幅度與峰值時序（且結論是仰式峰值**早於**自由式），沒有把延遲定義成策略，也沒有成績終點；樣本 10 名男性、無女性。
  - movement 落地：轉體時序可作為相位切分依據，但不得產出「練延遲旋轉」的 intervention。
  - 根因：個案（術語撞名）；同時是本批**唯一因蒐證誤讀而差點被錯殺**的條目，值得記進協議案例。
...
- 裁決：**修正（保留數值，但降級並標座標）**｜衝突型 1（座標系未對齊）
  - 判定：45° 與 40° 看起來互相印證，其實是兩個不同的量。Race Club 的 45° 是**掌面 pitch 的教學口令**，且下在 **lift phase**（與原文的「外掃期」不是同一相位詞彙）；Martens & Daly 2012 的約 40° 是轉引 Arellano 的 **sculling angle of attack**，即掌面與水流相對速度的夾角，且該研究是螢光染料質性視覺化、11 名受試者做**分析式模擬動作**、非比賽速度、無掌面三維角量測。兩者參考座標不同，不得互相支撐。依型 1 解法：不選邊，把差異顯性化。
  - 可寫入：「教材建議外掃／lift 期掌面約 45° pitch（Race Club，教練口令層級）」+ 明標「非量測值；與 sculling 文獻的約 40° angle of attack 座標定義不同，兩者不可互引」。相位名稱需對齊教材（lift）或註明對應關係。
  - 不得寫入：把 45° 當量測結果；用 Martens & Daly 為 45° 背書。
  - 本層限制：第 2 層蛙式 0 篇，查無不具推論力（協議 §1）；本條的第 3 層命中亦非競技蛙式完整週期，所以「無一手量測」是實況而非搜尋不足。
  - movement 落地：角度值不進 demand。若要寫，只能寫「外掃期需掌面 pitch 控制」的定性需求，不掛數字。
  - 根因：**結構性**——見文末結構性根因 2（數值跨座標系搬運）。
...
- 裁決：**修正（兩處）**｜①屬同層來源優先序裁決，②屬效益句代換
  - ①「在下巴／胸前併攏**後**迅速前推」→ **改為移動中靠攏**。教材直接給了不同的時序：`42_Figure_4.1.md:59` “the hands, which will come together **in the middle of the arm recovery as the hands are moving forward**”，並另有 `:61` “The hands should not clap together.”。原文把靠攏寫成前推的**前置事件**，教材把它寫成前送**過程中**發生；且「併攏」一詞逼近教材明文反對的 clap。
  - ②「划距效益」→ **刪除，且方向相反**。教材唯一給的效益是 `21_Chapter_18...md:74` “puts the hands back in front sooner, so it enables a much faster **stroke rate**”——那是**划頻**，不是划距。划頻與划距在 rate–length 取捨上是相反端，把教材的划頻效益改寫成划距效益，是把來源講的話換成了它的反面。Strzała 2013 只有 recovery 相位百分比與 V50 的相關（r=0.54），23 名 15 歲男性，不能推因果，也未標定靠攏事件。
  - 可寫入：「前送期雙手於移臂中段、在向前移動的過程中靠攏，快速前送至伸直；雙手不應拍合」+「教材主張此做法使手更早回到前方，因而支持較高划頻（Race Club，非控制實驗）」。
  - 不得寫入：先併攏再前推；任何划距／stroke length 效益。
  - 根因：**結構性**——結構性根因 1（效益句自由代換）的最嚴重案例：不只是無來源，是把來源的結論換成相反的量。
...
- 裁決：**修正 + ⚠︎ 缺陷 3 確認成立**｜衝突型 1（術語層級混用）
  - 缺陷 3 裁定：**成立**。原註記的疑慮正確——掌心朝向是肱骨旋轉與前臂旋前／旋後的合成結果，三層來源沒有任何一個量測前臂相對肱骨的旋轉（Pinto 2025 只用 hand entry／exit 劃週期）。本條只能停在空間朝向層，不得產生任何前臂旋轉的解剖敘述或肌群需求。分類維持 C。
  - 內容裁定：「掌心朝下」→ **刪除**。教材是直接比較而非查無：`24_Chapter_21...md:104` “entered his hands pitched with the **thumbs down slightly, which causes less drag than with the palms down**”，`44_Figure_5.1.md:11` 也給 thumbs-down 指令。原文把教材明指較差的選項列為第一選項。「些微朝外下方」與 thumbs-down 大致相容，但應直接改用教材原語。
  - 可寫入：「入水時拇指略朝下（掌面因而略偏外側）」（Fundamentals of Fast Swimming Ch.21；Science of Swimming Faster Figure 5.1）+ 標「教練慣例層級；教材以 Michael Phelps 為例，無樣本、座標系或統計」。
  - 不得寫入：「掌心朝下」；把 “causes less drag” 當量測結論（教材未附任何阻力量測）；由掌向反推旋前／旋後。
  - **欄位紀律（採自 `plans/關節主張裁決_蝶式.md` 附錄 A）**：掌心朝向只能放**池畔可見**欄位；關節欄位必須分別填「肩：內／外旋」與「前臂：旋前／旋後」。依據 Neumann `018:692`（旋前旋後以橈尺關節繞前臂縱軸定義）、`017:1028`（肱骨軸向旋轉繞肱骨長軸定義）、`028:799`（骨動學命名以解剖姿勢為基準，不由空間朝向反推）。本條的三層游泳蒐證與該附錄的解剖裁決**結論一致**，互為佐證。
  - 本層限制：第 2 層蝶式 0 篇，查無不具推論力。
  - 根因：**結構性**——結構性根因 4（朝向反推旋轉）與根因 6（掌向憑直覺填空），與 BK-17、BF-25 同錯。
...
- 裁決：**修正（換名 + 改順序）**｜與 BK-26 同屬命名問題，但**不是型 5 分歧，是單方無來源**
  - 判定：與 BK-26 的差別要說清楚——BK-26 是多套有來源的命名並存（並列處理），本條是原文用的名稱在三層**完全查無游泳來源**，而同一本教材給了有出處的替代名稱。無來源方不進入並列，直接換掉。`50_Figure_5.7.md:1` 明確命名為 **hourglass**：“The **hourglass pattern** scribed by the hands from the commencement of the (a) outsweep, (b) insweep, (c) upsweep, and (d) release.”「keyhole」的線上檢索 5 筆命中皆為非游泳同義詞雜訊。
  - ②順序：教材給的是**依序**不是同時——`24_Chapter_21...md:106` “a very brief and short outsweeping motion ... **followed by** a pressing downward of the palms **then** a backward motion”。原文「向外、向下、向後掃出」若被讀成同時發生即為錯。
  - 可寫入：「外掃為短促的向外動作，隨後轉為掌心下壓、再向後」+「整體手路徑在教材中命名為 hourglass（outsweep→insweep→upsweep→release）」，並標明這是路徑形狀模型，非三維軌跡量測。
  - 不得寫入：「keyhole」；外／下／後同時發生；由 outsweep 直接宣稱推進（Fernandes 2024 只把 outsweep 當分析相位，未量分相推力）。
  - 本層限制：第 2 層蝶式 0 篇，查無不具推論力。
  - 根因：個案（命名未溯源），同結構性根因 5。
...
- 裁決：**拆分：①支持，②③刪除**｜非衝突，是比較級無對照組
  - ①「蝶式沒有軀幹旋轉輔助」：**支持**，且是本批教材支撐最直接的一句。`44_Figure_5.1.md:5` “unlike in front crawl, **cannot be assisted by a body roll** about the longitudinal axis”，`51_Figure_5.8.md:7` 同義重述。
  - ②③「菱形肌、前鋸肌負擔比自由式更大」：**刪除**。三層沒有任何同 protocol 的蝶式 vs 自由式比較。手上僅有的資料還指向反方向的讀法——Nuber 1986 說 “the serratus anterior functions near maximal muscle test during **each** stroke”，即各泳式都接近最大，不支持「蝶式更大」；Matsuura 2022 有 12 肌但只有蝶式、只分有無肩傷，無自由式對照。**菱形肌在三層中沒有任何游泳 EMG 資料**，連描述都不能寫。另外「負擔」本身未定義（幅度？積分 EMG？疲勞？力矩？），即使有資料也無法驗證。
  - 可寫入：①如上；另可寫「前鋸肌在各泳式划水中皆接近最大自主收縮（Nuber 1986，水中樣本 7 人，摘要未報泳式間統計）」——這是負荷描述，不是比較。
  - 不得寫入：任何「比自由式更大」的比較級；菱形肌的負荷主張。
  - movement 落地：①可產出「蝶式移臂缺少 body roll 協助」的相位條件；肩胛穩定需求可寫，但**不得標成蝶式特異或跨泳式排序**。
  - 根因：**結構性**——結構性根因 2（比較級無對照組）。
...
- 裁決：**整條不採用（降為未證）+ ⚠︎ 缺陷 3 確認成立**
  - 缺陷 3 裁定：**成立**，理由同 BF-05。Matsuura 2022 的標記只有 wrist／elbow／acromion／hip，無法重建掌面，也無法分離肱骨旋轉與前臂旋轉。本條不得產生任何前臂旋轉敘述。
  - 內容裁定：三段全部不成立或無來源——起點「朝內」三層零命中；過程「逐漸轉為」需要連續掌向量測，三層零命中；終點「朝下」與教材端點不符，教材給的是 thumbs-down（`44_Figure_5.1.md:11`、`24_Chapter_21...md:104`），且 BF-05 處教材明指 palms-down 阻力較大。所以本條**唯一有來源的部分只有終點，而那個終點是被原文寫錯的那一端**。
  - **與既有附錄對齊（重要，修正本裁決的措辭）**：`plans/關節主張裁決_蝶式.md` 附錄 B 已對本條下過解剖層裁決，且提供了本次三層蒐證查不到的關鍵資訊——移臂期掌心朝向的改變**有相當比例根本不是前臂旋轉造成的**，而是上舉必然伴隨的肱骨外旋（Neumann `017:1192`），而那個外旋正是原文在隔壁欄位（BF-23）漏填的。因此「整條不採用」只針對**原文的歸因與措辭**，不是說這個現象不存在：現象為真，被掛錯關節。修好 BF-23 的外旋欄位後，本條大部分內容會被自動吸收。
  - 可寫入：無（以本條 ID 為單位）。整條降 `unverified`，保留原文擋重犯。移臂期手部朝向的正確寫法見 BF-05 裁決與附錄 A 的欄位紀律。
  - 不得寫入：palms-in → palms-down 的過程描述；任何據此推出的**前臂**旋轉肌群需求。
  - movement 落地：**不落成獨立 demand**。移臂期的相關需求應寫成**肱骨外旋**（隨上舉發生），來源掛 BF-23／附錄 B，不掛本條；手部朝向留空並標 evidence-gap。
  - 根因：**結構性**——結構性根因 4 + 6，與 BK-17、BF-05 三條同錯。
...
|---|---|---|---|---|---|
| BK-26 | C | 1 | 修正（並列三套分期，不選邊） | Fernandes 2022 Methods；Race Club Ch.14:22；SoSF Ch.3 | 個案／根因 5 |
| FR-09 | C | — | 刪效益句，降 unverified | 三層皆無；`20_Figure_2.2.md:17` 歸因 body roll | **結構性 1** |
| FR-13 | C | 3 | 部分支持（腕穩定）+ 刪角度與效益 | Race Club Ch.7:89；Caty 2007 | 個案 + 根因 1 |
| FR-17 | C | 3 | 支持（姿勢層）+ 刪因果 | Suito 2017；Heinlein 2010:36 | 個案 |
| BK-10 | C | — | 強制修正（刪頻率語言） | McKenzie 2023 Abstract；Tovin 2006:38 | **結構性 3** |
| BK-17 | C | §4 同層優先序 + 3 | 修正兩處（腕改輕屈、掌向改用教材措辭） | `37_Figure_3.1.md:7,16`；Race Club Ch.14:46 | **結構性 6** |
| BK-21 | C | 3 | ①支持 ②降 🔵 解剖推導 | `39_Figure_3.3.md:17`；Fernandes 2022 相位定義 | 個案 |
| BK-29 | C | 1 | **支持**（蒐證期的「教材對撞」誤判不成立） | `40_Figure_3.4.md:25` 與主張同向 | 個案（術語撞名） |
| BR-06 | C | 1 | 修正（保留 45° 但標為口令、禁與 40° 互引） | Race Club Ch.18:24；Martens & Daly 2012 | **結構性 2** |
| BR-18 | C | §4 同層優先序 | 修正兩處（靠攏時點、效益由划距改划頻） | `42_Figure_4.1.md:59,61`；Race Club Ch.18:74 | **結構性 1** |
| BF-05 | C | 1 | 修正（刪「掌心朝下」）+ 缺陷 3 成立 | `24_Chapter_21...md:104` | **結構性 4 + 6** |
| BF-09 | C | — | 修正（keyhole→hourglass、改依序） | `50_Figure_5.7.md:1`；`24_Chapter_21...md:106` | 個案／根因 5 |
| BF-22 | C | — | ①支持 ②③刪除 | `44_Figure_5.1.md:5`；Nuber 1986 | **結構性 2** |
| BF-25 | C | — | 整條不採用（現象為真但掛錯關節）+ 缺陷 3 成立 | 三層零命中；終點與教材相反；蝶式附錄 B + Neumann `017:1192` | **結構性 4 + 6** |

計：支持或部分支持 8、修正 5、整條不採用 1（BF-25）；另 FR-09 保留原文但降 unverified。**零條原封不動通過**。

## 結構性根因（跨條目，非個案）

1. **效益句自由代換**（FR-09、FR-13③、FR-17、BR-18）：把一個姿勢或解剖動作直接接到成績／效率／划距效益，來源多半只到教材口令甚至完全沒有。最嚴重的是 BR-18——教材講的效益是**划頻**，原文寫成**划距**，是 rate–length 取捨的相反端。**這是本批最普遍的錯誤模式，4/14。**
2. **量化或比較主張缺對照／缺座標定義**（BR-06、BF-22）：BR-06 把兩個不同座標系的角度（掌面 pitch 45° vs sculling angle of attack 40°）當互相印證；BF-22 寫比較級「比自由式更大」但三層無同 protocol 對照，且僅有的 EMG 資料指向「各泳式皆接近最大」。
3. **負荷描述冒充流行病學**（BK-10）：與 `plans/關節主張裁決_C類傷害宣稱.md` 已證實的 3/5 傷害宣稱不符是同一模式。傷害領域的「聽起來合理」特別不可靠。
4. **由空間朝向反推關節旋轉**（BF-05、BF-25）：兩條的 ⚠︎ 缺陷 3 註記**經查證全部成立**，且與 `plans/關節主張裁決_蝶式.md` 附錄 A／B 的解剖層裁決獨立得出同一結論（Neumann `018:692`、`017:1028`、`028:799`）——一邊查游泳文獻、一邊查解剖教科書，兩條路都指向「掌心朝向不能當關節欄位」。附錄 B 另補上本次蒐證查不到的一層：BF-25 的掌心變化主因是上舉伴隨的**肱骨外旋**（`017:1192`），而那正是原文在 BF-23 漏填的欄位——**缺陷 3 常是缺陷 2 的症狀**：因為漏了關節欄位，才需要用空間朝向去間接描述。
5. **命名未溯源**（BK-26、BF-09）：兩條都採自單一教學來源。但處理方式必須分開——BK-26 是多套有來源的命名並存（並列、不選邊），BF-09 是單方零來源而同教材有替代名（直接換掉）。
6. **腕位／掌向憑解剖中立直覺填空**（BK-17、BF-05、BF-25）：本批新發現的模式，3/14。原筆記在缺乏查證時傾向填「腕伸直」「掌心朝下」這類中立／直覺值，而教材實際描述多為**屈腕、拇指朝下、掌心朝後**——**三條全部寫反**。凡遇腕位或掌向敘述，一律視為高風險，必須逐句對教材原文，不得沿用。

## 對後續批次的操作指示

- 第 2–4 批（ankle-foot／hip-knee／spine-neck）蒐證與裁決時，**先用上列 6 個根因當篩子過一遍**，命中者優先處理。根因 1 與 6 預期在其他分區同樣高頻。
- **蒐證階段標出的「教材對撞」不得直接採信**：BK-29 是本批唯一因蒐證誤讀而差點被錯殺的條目——教材的 `late rotation` 與主張的「推進段不轉」是撞名而非矛盾。裁決前一律逐句比對兩邊的**所指**，這正是協議 §3 警告的「把回答不同問題的東西當矛盾去裁決」。

## 對 W4（Step 17–18）的直接輸入

- **BK-26 是 blocker**：`canonical/movement/` 的相位 ID 必須帶「出自哪套分期」的欄位；仰式若採四 sweep，移臂段將整段無 ID，W4 前須先決定補 recovery 相位或改採六相。
- **可直接產出 demand 的只有 3 條**：FR-13②（腕屈伸肌共收縮，直接游泳 EMG，本批證據最強）、FR-17（高肘構型，相位邊界用 Heinlein 2010）、BF-22①（蝶式移臂無 body roll 協助，教材直述）。
- **明確不得落成 demand／intervention**：FR-09、BK-10（僅能 `evidence-gap` + `do-not-prescribe`）、BK-29（不得寫成訓練指令）、BF-25。
- **數值一律不進 demand**：BR-06 的 45° 只能以定性需求形式存在。
