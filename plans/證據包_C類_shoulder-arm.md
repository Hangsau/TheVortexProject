...
- 裁決：**整條不採用（降為未證）+ ⚠︎ 缺陷 3 確認成立**
  ...
  - 可寫入：無。整條降 `unverified`，保留原文擋重犯。若要寫入水前手部朝向，直接用 BF-05 的裁決結果，不從本條走。
  - 不得寫入：palms-in → palms-down 的過程描述；任何據此推出的前臂旋轉肌群需求。
  - movement 落地：**不落成 demand**。移臂期若需條目，只能寫肩與肩胛的需求，手部朝向留空並標 evidence-gap。
...
| BF-25 | C | — | 整條不採用 + 缺陷 3 成立 | 三層零命中；終點與教材相反 | **結構性 4 + 6** |
...
4. **由空間朝向反推關節旋轉**（BF-05、BF-25）：兩條的 ⚠︎ 缺陷 3 註記**經查證全部成立**。掌心朝向是肱骨旋轉＋前臂旋轉的合成，三層沒有任何研究分離量測過這兩者。
